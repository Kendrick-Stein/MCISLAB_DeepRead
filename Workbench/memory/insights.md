...
- **confidence**: medium
- **source**: cross-validation
- **impact**: RL-based GUI Agent Training（reward/数据设计的低竞争切入点）、Agent-Facing Environment Runtime（环境应暴露 failure evidence 作为 affordance，使 harness 能把错误转成可执行恢复策略）、[[Topics/GUIAgent-Survey]] Takeaway 14、[[Topics/RealWorldGUIAgent-Reliability-Survey]]
- **status**: provisional
