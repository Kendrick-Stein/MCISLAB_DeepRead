...
- **2026-09-18｜头条增益里有相当一部分经不起零成本对照，09-11 记下的"外挂治理缺降级对照"只是其中一个切片**：九处互相独立的读数形状一致——把学出来的组件换成零参数的平凡规则、把外部环路整个拿掉只留它带来的额外条件、把适配过的模型换成冻结骨干加一句朴素提示，增益就大幅缩水甚至消失。最干净的三个：[[Papers/2608-PRACTICE]] 里不含任何学习的 BPE 技能库单项贡献 +16.4，而全部学习阶段合计只有 +9.0；[[Papers/2609-SparseResidualWM]] 的稀疏残差预测在被推动物体上与 no-op 基线持平，即该论文的核心机制在它最该起作用的样本上没有净效应；[[Papers/2609-PhysBrain15]] 把统一动作词表训到 8B、报满 28 个理解基准，却零消融、零闭环成功率，因而无法判断词表设计贡献了什么。与 [[Papers/2608-Zeva]] 合看更清楚：它在完全不更新梯度的前提下把累计成功率从 26% 推到 73%，说明"改进落在权重之外"已经是一类稳定可行的取法，而恰恰是这类取法最难与"只是多给了执行预算"区分开。举证责任因此要反过来落：凡外挂环路、技能库、记忆或 critic 带来的增益，都须先报同预算的冻结基线。边界有两层——这些读数全部来自各自论文的自报数据与自陈缺口，尚未见独立复现；且"组件无效"与"基准里没有需要该组件的实例"两种解释目前无法分开（[[Topics/EmbodiedAI-Survey]] §Key Takeaways 13 / §Open Problems 14、[[Topics/VLA-Survey]] §横切议题四）
- **2026-09-18｜09-07 记下的"接口形状决定预测通路净效应"要补一条作用域：那些结论是在小预算下选出来的，会随预训练规模翻转**：[[Papers/2609-OpenWAM]] 把 world-action model 的设计空间写成受控变量逐项扫描，最优的训练期注意力掩码在具身预训练前后互换——从零训练时"动作看视频"这一路略优（92.39），做过预训练后互相可见的配置在三个 split 上全胜。同一份扫描里另有两条反转：16 种异步去噪配置全部输给同步对角线（最好的异步 92.3 对同步 93.0），而生成式主干仍按参数量单调买分（1.3B 90.14 → 14B 93.79），后者把"data recipe 重于 model size"限定回长在 action-labeled 数据上的 VLA。[[Papers/2609-ZETA]] 从另一侧给出同样量级的提醒：state/action 参照系这一个选择值约 15 分（60.3±1.1 对 75.7±1.3），大于该节讨论的多数架构选择，且它同时暴露 zero-shot 定义本身在分歧中——迁移到新本体须拆成 strict 与 pretrain-exposed 两档，此前跨论文并排的 zero-shot 数字不在同一档上。这类"受控扫描"正在成为一种独立产出形态，交付的是"在什么条件下选什么"而非新方法；共同弱点是单点估计——无种子、无误差棒，故上述反转的统计强度有限（[[Topics/EmbodiedAI-Survey]] §路线 3 / 路线 5、[[Topics/VLA-Survey]] §convergence 观察 14）
- **2026-09-18｜成功判定器从评测的背景设施变成被测对象，判据本身要当实验条件报告**：09-07 由 [[Papers/2608-WorldSimProbe]] 记下的"judge 判定规则须作为实验条件报告"此前只覆盖 world model 侧，现在延伸到策略成功率本身。[[Papers/2609-FailBench]] 把 2,197 段操作视频收成单一判定任务，最好的通用模型只有 0.77 macro balanced accuracy；[[Papers/2609-DroneCATS]] 的判据审计更直接——换一条成功判据会翻转 182 次成功里的 95 次或 58 次，而 [[Papers/2608-ZeroWAM]] 则实际改写了两个 RoboTwin 任务的判据。含义是所有策略侧基准分数都带着一个此前未被计量的误差源，其量级足以覆盖多数论文报告的方法增益；"判定器与策略联合评测"尚未见对应工作。同一组证据还把长程失败的定位往前推了一步：[[Papers/2608-MNISTPro]] 的离线画布反事实把失败点定在观测整合而非注视点选择，[[Papers/2608-UrbanGround]] 上十个模型城市级长程导航仅 0.0–3.8% 且局部感知不随探索长度复合，因此获取 / 整合 / 终止三个环节须分开计分。边界：FailBench 笔记核为 partial，其 Table 2 与 Table 8 在全部 14 个子集上系统性不一致，绝对值应保守读（[[Topics/EmbodiedAI-Survey]] §Overview 第 9 条 / §路线 6 / §专题一 A5）
