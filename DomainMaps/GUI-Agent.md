...
- **[2026-07-19] 评估器可靠性获得定量下界**：[[2504-AgentRewardBench]]（1302 条专家标注轨迹测 12 个 judge）：LLM judge precision 无一超 70%、rule-based recall 仅 55.9%、副作用检测 precision 7–14%——评估的双向失败被首次系统测量，与 CUARewardBench（ORM 82.9%）/WebJudge（~85%）共同框定 judge 精度边界。见 [[Topics/WebAgent-Survey]] Takeaway 5。