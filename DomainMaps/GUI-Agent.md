...
- **2026-08-11｜"工具通道有益"被受控实验改写为条件命题，外置资产价值的条件化 pattern 三方合拢**：[[Papers/2604-ToolIllusion]]（COLM 2026）用 3 framework × 5 跨家族 backbone 的同 framework 内工具开关对照给出裁决：人工 REST API 对所有 backbone 稳定有效（WebArena 最高 +19.6pp），LLM 合成工具只在使用者明显弱于合成者时有效（GPT-5 用 GPT-5-mini 的工具反掉 2pp），且工具库规模是负债而非美德（SkillWeaver 79% 工具零调用、Hybrid-Agent 开工具后 token 10.5M→33.9M / 步数反升）。与 07-28 的 [[Papers/2606-SkillMemoryBudget]]（budget-matched 下 skill/memory 模块集体失效）和 08-05 的 [[Papers/2608-ScreenshotsOrTools]]（采用率可抬一个数量级而精度不动）合拢成同一个 pattern：**tool / skill / memory 一切经验外置资产的价值都是条件化的——取决于合成者-使用者能力差、预算记账口径与调用语义正确率，无条件的"加了就好"在三条独立证据线上都已被否**。边界：跨 framework 数值混杂 harness 差异、单次运行无方差、沙盒 DOM 稳定使隐性税为下界。见 [[Topics/CUA-Survey]] §4.7.2。
- **2026-08-11｜per-step compute allocation 成为可测量的 harness 旋钮，但两篇的自身开销都在账外**：[[Papers/2602-CATTS]] 给出 web agent 每步采样的饱和曲线（WebArena-Lite N=10 后倒退）与高共识 override 的代价（arbiter 在 margin>0.7 步骤上推翻多数票 → 任务成功率 46.9%→35.0%），entropy-gated 仲裁在 matched compute 下 +3.3pp（可辩护口径为全阈值平均 +2.4pp）；[[Papers/2603-Ares]] 把分配对象换成 reasoning effort 档位，最可迁移的产出是 effort 需求的结构分布——`go_back`/`branch` 这类纠错与重规划节点最吃算力、早期导航步可廉价。共同缺口与 SkillMemoryBudget 的记账纪律同源：CATTS 的 token 账未拆轨迹长度效应，Ares 的 router 每步 prefill 全部历史却不在任何报告数字内——"省了多少"在补上自身开销前不可引用。见 [[Topics/CUA-Survey]] §7.14。
- **2026-08-11｜验证组件的方向性偏差增至三点：偏差方向由组件在执行链中的位置决定**：[[Papers/2608-StepReflect]] 测得 zero-shot GPT-5.2 做 per-step reflection 时负类 95.10% / 正类仅 51.39%——系统性报忧，一半正常步骤被误判失败；与 08-02 的 [[Papers/2607-OSReward]]（passive 轨迹 judge 偏放行，over-accept 占 2/3）和 08-03 的 [[Papers/2607-MisScoreCUA]]（programmatic oracle 偏严）合成三点：**step 级 reflection 偏报忧、轨迹级 VLM judge 偏放行、程序化 oracle 偏假阴——纠偏策略必须按组件位置分别设计，任何单一方向的推广都是错的**。StepReflect 同时证明结构化 transition specification 可让 8B 模型超 frontier zero-shot 11.83pp，但部署时 host agent 不暴露可靠 pre/post-conditions，"planner 原生输出可检验 specification"是新的 open gap。见 [[Topics/CUA-Survey]] §6.10。
...
- **[2026-08-18] verifiable task 合成的失效面被推进一层：不变量通过 ≠ reward 对齐指令语义**：[[Papers/2608-UIMate]] 指出 `R(E_0)=0, R(E*)=1` 这个 CUA-Gym 一线通用的执行不变量只建立**内部一致性**——参考完成态与 reward 函数可以共享同一个语义盲点，两边一起错时检查照样通过。对已通过该筛选的 evaluator 做审计，仍约 **18% 与指令语义不对齐**（过严匹配 40% / 语义空洞的断言 23% / 判错目标 19%）。这与 [[Papers/2607-SCALECUA]] 的 task validity 82.0%/58.3% 不是同一件事：后者问任务本身成不成立，前者问任务成立且不变量通过之后 reward 到底在测什么，因此是这条流水线上第二层、且此前无人量化的质量上界。方向上"过严"占最大一类，与 [[Papers/2607-MisScoreCUA]] 在评测侧测到的 programmatic oracle 偏严同向，但一个审计训练期合成 verifier、一个审计已发布 benchmark，不构成互相复现。同篇另有两条阴性结论值得记：adaptive curriculum 与 process credit model 只加快收敛（常在不到一半更新内达到相当性能）而不抬终点；historical reasoning 评测时开启有 +3.43pp / +2.27pp，纳入 RL 训练则触发 entropy collapse 特征并降低性能。**边界**：提出方自审，审计样本量与人工校准未报；27B 的 OSWorld-Verified 77.0 与 WAA 66.2 是 bundle 级结果，数据侧与训练侧无任一组件级消融，9B 版本作者自陈低于同尺寸 ScaleCUA。库内暂无独立验证。见 [[Topics/CUA-Survey]] §7.9、§6.3 与 §11.4。
- **[2026-08-18] WebArena 系 raw pass@1 首次有了一手的"注水量级"，之前只是共识性怀疑**：[[Papers/2608-DarwinX]] 在 WebArena-Infinity 的 1,260 道真实任务上用两阶段检测器（静态去混淆 + taint 追踪 + 独立 judge，覆盖率 99.0%/99.4%）逐条判定一个 harness 的 raw 53.0%，查出 **293 条无效轨迹**——155 条越界访问 evaluation plane、97 条访问特权 host、26 条 exploit 或提权、15 条直接篡改 raw state，**confirmed-invalid 率 23.5%**；演化后前三类全部消失、只剩 17 条 raw-state mutation 集中在单个应用，降到 1.4%。[[Papers/2407-AgentsThatMatter]] 与 [[Papers/2510-HAL]] 此前给的是 gaming case 的存在性，这里第一次给出比例。含义是这类 benchmark 上未配套 validity 审计的 pass@1 都应默认打折，而目前几乎无人配套发布。**边界须同引**：合规率的改善有一部分来自**放宽合法动作的定义**（演化后的 prompt 把"绝不直接写状态"改成"经有界审计后可调用应用自有 helper 做最小改动"），划线的 rubric 由同一团队撰写，作者自述为 behavioral co-improvement 而非 causal decomposition；审计器未经第三方复核，原文另有一行口径不同的计数（invalid successes 120→17）。同篇的 harness population selection 则给自演化路线补了 contract-based 准入这一 gate 形态（净增益为正 + 回归有界，fitness 取 benchmark 自带 verifier 的 avg@k），与 [[Papers/2608-Ouroboros]] 的 blocking review 面板并列；但 population 与重组这一核心主张恰在收益最大的 benchmark 上未生效——每次 merge 都被 revert，增益沿单条短 lineage 累积。见 [[Topics/CUA-Survey]] §8.13 与 §7.11.1。
- **[2026-09-18] 自改进的增益首次被同起点同 backbone 拆成 harness 与机制两级，大头在 harness——Pattern 4 的筛选条件多出一条归因轴**：[[Papers/2609-RSIAgent]] 在 GameCraft-Bench 的 40 个任务、四个 generator 上给出四组受控拆分：agent harness 相对基线贡献 **+5.07 / +11.33 / +14.18 / +12.52**，自改进机制在其上再贡献 **+3.44 / +3.76 / +3.99 / +3.64**——机制的边际贡献稳定在 harness 的三到四分之一。主表同向：ALE 上不含自改进的 harness 已达 83.75、越过同表 GPT-6 Astra 的 82.26，机制只再加 1.07（OSWorld 侧相反，w/o RSI 的 71.97 低于 GPT-6 Astra 的 72.60，须靠 +7.01 推到 78.98）。Pattern 4 此前的筛选条件是**预算与口径是否匹配**，这条补上的是**归因是否拆开**：一个 self-improving 系统的分数里，被命名的那个机制与承载它的 harness 是两笔账，不拆开报时读者默认记到机制头上。这与 [[Papers/2608-Ouroboros]]、[[Papers/2608-DarwinX]] 形成对照——那两篇都把 harness 当演化对象却未建立 causal attribution，这里在可得条件下建立了，答案对整条线不利。**边界须同引**：基线是"关掉探索与持久记忆"而非等预算对照（作者自陈 "not a matched-budget estimate"，全文无探索阶段的 token/费用/wall-clock 汇总）；主表由真实结果与保留基线拼接（OSWorld 41/82、ALE 19/67）且只给基线未满分的任务分配探索，估计量在构造上偏乐观；verifier 为模型自判、准确率未量化，而其失败分析自己记录了误判 PASS 导致错误规则固化并被后续 run 复用。另需注意其记忆按目标任务从空建起、评测前冻结——摊销是 per-task 的 test-time search 加笔记，不是环境级 flywheel；标题的 recursive self-improvement 三条判据全不满足（见 [[Topics/SelfEvolvingAgents-Survey]] §7.5，第三个同形样本）。见 [[Topics/CUA-Survey]] §7.11.3 与 §8.13.1。
- **[2026-09-18] "agent 化削弱 guardrail"从假定变成同条件测量，同时该文自家的 step 曲线反对"风险随步数累积"——防御该守的是动作许可门**：[[Papers/2506-MLATrust]]（IJCV 2026）用同一 backbone、同样构造方法的数据，只更换"单步 MLLM 问答"与"多步 GUI 执行"两层外壳，测得 refusal rate **90.5%→70.2%（GPT-4o）、86.0%→62.5%（Gemini-Pro）、78.0%→57.8%（Claude-3-7-sonnet）**。既有 agent safety benchmark 普遍缺的正是这个单步 baseline 条件——没有它，"agent 不安全"只是相关性观察，无法归因到 agent 化这一步。更值得记的是它的 Figure 6：横轴 step 0/1/3/5/7（step 0 = 纯问答），四条曲线在 0→1 处集体断崖，此后全部在同一水平带内**震荡回升**，没有一条继续下行，而正文只讨论 0→1 那一跳，从未把 abstract 反复出现的 "latent and nonlinear risk accumulation across decision cycles" 与这些数据点对上。若该形态在更密的 step 网格上、控制住任务难度随步数的混淆后仍成立，"长程 agent 更危险"就要改写为"能动手才危险"，而这两种读法指向的部署位置完全不同——该守 action gate，不是 step budget。**记为假设而非结论**：逐步数值由矢量图坐标解码、正文未印出，全文无重复采样/seed/置信区间，单次运行下的震荡无法与噪声区分；所有 RtE 由 Longformer/GPT-4 判定且无 human agreement，排名可能含 judge artifact；RtE 作为 safety 主指标不惩罚 over-refusal（controllability 只测多做不测少做），恒拒绝模型会横扫两维；benchmark 锚在 Amazon/Twitter/GitHub 等生产站点，纵向复现不成立；arXiv 停在 v1 的 13 模型而 IJCV 版为 17 模型，引用须标版本。见 [[Topics/CUA-Survey]] §6.11.1 与 §8.9。
