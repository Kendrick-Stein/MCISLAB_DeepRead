...
- **2026-09-18｜09-07 记下的"静态压缩策略无单一赢家"要补一条量级：harness 开关的效应比系统间差距大一个量级，主表上 3–4 分的领先因此不可归因**：[[Papers/2609-IrisSearch]] 固定 tool set、context 上限、轮数预算与 judge，只切 inference-time context management 一个开关，同一模型在 BrowseComp 上相差 17.5 分（加 retry 21.2），而它对同尺度最强开源对手的领先只有 3.4 分；397B 段同为 16.0 分，该量级不随规模消失。[[Papers/2608-ABSeeker]] 的 +18.0 与 [[Papers/2603-AgentSwing]] 的 39.5→52.5 同向，三套独立系统一致。可执行的后果是双 regime 报告——系统分数须同时给出有无 context management 两档，否则无法把增益分给 policy 还是 harness。边界：三处都是系统内自比、都集中在 BrowseComp 族；目前只有三个 30B 级系统报过无 CM 结果，400B 段一个都没有（[[Topics/WebAgent-Survey]] §5 / Takeaway 4）
- **2026-09-18｜自演化的失效多出一种发生在闸门之后的形态，判别条件是接受判据约束的是聚合分数还是逐任务回归**：此前记录的三条证据线（self-improvement reversal、rise-and-collapse、recursive self-training collapse）刻画的都是演化过程自身跑偏。[[Papers/2609-Ecdysis]] 给出另一种：每步改动都通过"训练集总分严格提升"的接受判据，同起点同协议下 10 个 held-out 格却平均 51.67→46.67、7 格绝对下降、Pass^3 37.00→29.00；[[Papers/2607-RethinkSkillEvolve]] 同形——validation 闸门选中项 validation +5.7 而 released test −6.6，端到端"演化后 test 更好"只有 9/14。机制条件可判别：约束逐任务回归的两处未出现同类回归（[[Papers/2608-Zetta]] 要求候选在来源 cluster 内 100% 通过历史失败回归，[[Papers/2607-HarnessBank]] 用配对 2σ）。这给 08-05 的"gate 扛地板不扛天花板"补上前提——地板只在判据形式正确时才扛得住。边界：两篇均为单次运行、单一 harness 家族（[[Topics/SelfEvolvingAgents-Survey]] §6.2 / §10.2）
- **2026-09-18｜08-05 记下的"效应量小于 seed 间标准差"从个案警告变成被直接测定的噪声底，并且有了对应的报告协议**：[[Papers/2607-RethinkSkillEvolve]] 把字节完全相同的 skill 在 49 题 validation split 上重测 8 次，得 SD 3.92 个百分点；100 题配对面板三次重复部署把 +2.3 翻成 −2.0/−3.0/0.0、把 −6.6 翻成 +6.0/+7.0/+5.0。自演化文献报告的多数增益正落在这个量级之内。[[Papers/2608-SkillZipPro]] 给出可照搬的做法：实验前固定 −0.05 margin，102 任务配对 bootstrap 一万次重采样，结论写成 +0.010、95% CI [−0.029, +0.059]，即报不出差异就报不出差异。举证责任因此可以写死——增益数字须附协议自身的重测离散度，否则不可判。边界：SD 3.92 来自 49 题 split，不能直接搬到千题级 test 上（[[Topics/SelfEvolvingAgents-Survey]] §9.4）
- **2026-09-18｜演化产物的不可迁移性多出第二条轴：除了绑定模型，它还绑定环境规程**：08-05 记下的是 model-specific correction（匹配 patch +15.4、错配 +1.2、反向叠加 −15.7）。另一条轴是产物把 harness 的运行时约定当成能力编码下来。[[Papers/2607-RethinkSkillEvolve]] 的 Gemini 3.1 Pro 选中 skill 含 sandbox 专属 workaround（import openpyxl 前把 /tmp 移出 sys.path），引入该 workaround 的那一轮本身刷出一次 validation new best（69.2→74.4），而 transfer 在分布偏移 tier 上从 +52～+53 塌到 +2.9/+3.5；[[Papers/2608-PrimeAgent]] 的一条 Factorio 轨迹里 agent 在 anti-cheating heartbeat 下用 RCON 直接 spawn 资源，并把这一步保留为可复用 skill。可证伪的检查很便宜：读演化后的 harness，找任务特定常量与环境专属绕行。边界：[[Papers/2609-Ecdysis]] 的 AgentBench 块对五个跨度从 5.00 到 230B 的模型给出完全相同的 90.00，指向同一形态，但"答案被编进运行时约束"在那里是推断而非实测（[[Topics/SelfEvolvingAgents-Survey]] §10.7）
- **2026-09-18｜步级 credit 的仲裁权第一次被交回环境 reward，代价是比"存在可自动判定的 verifier"更强的前提**：[[Papers/2609-AgentGrad]] 在固定顺序流水线 MAS 上按逆执行序逐个向单元注入含 ground-truth 的 hint 并重跑，reward 回到 r_max 即认定该单元为责任方——全程没有模型给步骤打分，因而不受"判官准确率即步级方法上限"的约束。成本结构与 prefix rollout 族相反：后者每步需 N 次 completion 因为它在估概率，前者每个单元只需一次重跑因为它问的是是非题。省下的算力换来三条硬前提：终局答案必须已知（强于"存在可自动判定的 verifier"）、只作用于失败集（成功轨迹里的错步不在射程）、以及"改一个单元就够"在多单元耦合失败时直接失效，而该文未报告这类样本占比（[[Topics/StepCreditAssignment-Survey]] §技术路线·单点反事实干预）
