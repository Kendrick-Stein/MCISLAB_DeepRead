...
repo_analyzed: 9eb28bfd8e4ce05a3dd1c23dd05b4bb85f19f16b
---
## Summary
...
> repo: https://github.com/neoteai/N0-TWAM @ 9eb28bf，分析日期 2026-08-06，静态分析未执行代码

仓库已于 2026-07-31 公开（单个 "Initialize" commit，工作树 35 MB / `.git` 17 MB，CC-BY-NC-SA-4.0），论文与本笔记原记载的"将会公开"已过时。

**架构**：`n0_twam/models/mot.py` 是 MoT 主干——每模态一个私有 `MoTExpert` 栈（`mot.py:L143-196`），每层只经一次 `SharedSelfAttention`：三路 q/k/v 拼接后做单次 attention 再切回各专家（`mot.py:L298-361`），窄专家把 1024 的私有残差宽度投影进 3072 的共享 attention 宽度。`n0_twam/models/model.py` 承载其余全部机制：FlexAttention mask 构造（frame-id cascade）、触觉编码与输出头、LocalTactile cross-attn，以及 `forward_train` / `forward`（推理带流式 KV cache）两条路径。`n0_twam/train.py` 是 FSDP2 训练器（video/action/tactile 三路 flow-matching 损失），`n0_twam/n0_twam_server.py` 是流式 websocket 推理服务，`script/` 做 LeRobot → Wan VAE latent 的离线编码。该仓库只发布模型、推理服务与 post-training 工具，明示不含大规模预训练管线（`README.md:L20`）。

**论文 ↔ 代码对照**：

| 论文 claim | 代码位置 | 一致性 |
|:--|:--|:--|
| frame-id causal cascade 完全由一张 attention mask 实现，不改 attention 算子 | `n0_twam/models/model.py:L148,L167,L328-335` | 一致：video/tactile frame_id = `(f//chunk)*2`，action = `(f//chunk)*2+1`；mask = OR(clean2clean∧causal, noise2clean∧causal-exclude-self, noise2noise∧same-frame) ∧ seq ∧ window |
| MoT 三专家共享单层 attention，action/tactile 为 slim 专家（私有 1024 vs 视觉 3072） | `n0_twam/configs/twam_base_cfg.py:L106-107`；`n0_twam/models/mot.py:L143-196` | 一致 |
| action / tactile 专家从零训练 | `n0_twam/models/utils.py:L145-151` | 一致：warm-start 只拷贝 full-width 专家，narrow 专家被跳过并保留随机初始化 |
| predicted 通路以残差形式预测未来触觉 | `script/encode_tactile_latent.py:L7-13` | 一致，但残差定义是 `frame[t] − frame[0]`（对 episode 首帧），非对上一帧；论文只说"残差形式" |
| 触觉与视频共用同一冻结 causal video VAE 潜空间 | `n0_twam/models/model.py:L1130-1192`；`n0_twam/n0_twam_server.py:L549-557` | 一致：两路触觉都是 Wan VAE latent |
| observed 通路：冻结 estimator 出三轴力图 + contact mask，NeoForce（DINOv2 初始化 ViT-B）编码 | `n0_twam/n0_twam_server.py:L549-557`（观测触觉 = 原始触觉 RGB 归一化到 [-1,1] 后过 Wan VAE）；全仓 grep 无 NeoForce / DINOv2 / force-map 相关代码 | 不一致：该版本的 observed 通路停留在 VAE latent 空间，没有 force-space 编码器 |
| observed 通路经零初始化 cross-attn 注入 action head 之前 | `n0_twam/models/model.py:L1052-1053, L1881-1893` | 一致：`to_out` 权重与 bias 均 zero-init，cross-attn 位于 `action_proj_out` 之前 |
| 分阶段训练：预训练只开 predicted，post-training 才打开 observed | `n0_twam/configs/twam_base_cfg.py:L48-51`（released ckpt `use_local_tactile=False`）；`n0_twam/configs/twam_posttrain_cfg.py:L47` | 一致 |
| condition dropout 对触觉用"缺席"而非置零 | `n0_twam/models/model.py:L1522-1537`；`n0_twam/train.py:L468-475` | 一致：drop 时触觉 token 长度直接置 0、`local_tactile_tokens=None`，与真正无触觉数据同一条码路 |
| 动作回归 chunk-start 锚定 delta，逐元素相减而非 SO(3) 相对旋转 | `n0_twam/dataset/lerobot_latent_dataset_pi05_delta.py:L293,L303,L326-329` | 一致：SO(3) 相对旋转分支存在但由 `pi05_rot6d_relative_delta=False` 默认关闭（`twam_base_cfg.py:L73`） |
| （正文主推 delta 动作表示） | `n0_twam/configs/twam_posttrain_cfg.py:L42` | 论文未提：发布的 post-training 默认 `_ACTION_MODE = "absee"`（绝对末端位姿），注释称其为 "the final validated recipe"；论文只在真机 4 个任务上提到用绝对 EE |
| tactile punctuation：接触事件切子任务 + 推理侧 scheduler（predicted 触发 / observed 确认） | 全仓 grep 无对应实现；`n0_twam_server.py` 的 `_infer` 无子任务队列或阶段状态机 | 发布版未含该模块 |

**论文没写的实现细节**：

- **触觉损失少一层 timestep 加权**：video / action 的 MSE 都乘 `training_weight`（`n0_twam/train.py:L500, L533`），触觉是裸 `mse_loss`（`n0_twam/train.py:L604-605`）；权重键 `tactile_diffusion_loss_weight` 在任何 config 中都不存在（`n0_twam/train.py:L611`），恒取默认 1.0。README 说的 1:1:1 在实现上还叠了这条不对称。
- **训练期随机化窗口，推理固定**：每步随机采 `chunk_size ∈ [1,4]`、`window_size ∈ [4,64]`（`n0_twam/train.py:L453-454`），而推理固定 `attn_window = 72`（`n0_twam/configs/twam_base_cfg.py:L22`）。两者同为 frame-id 单位，KV pool 按 `attn_window // 2 = 36` 个 chunk 开（`n0_twam/models/mot.py:L534-541`），略宽于训练见过的最大 32 chunk——**待核**：是刻意留余量还是超出训练分布。
- **post-training 关掉了"缺席式 dropout"**：`twam_posttrain_cfg.py:L123-126` 把 `tactile_cfg_prob` / `cfg_prob` / `noisy_cond_prob_tactile` 全部强制为 0，注释理由是 MoT 的 tactile expert 不能接受空 token 序列、空专家会让 FSDP hang。即论文强调的 dropout 统一机制在单任务 post-train 配方里实际不启用。
- **CFG drop 时的零锚点**：drop 分支用 `_zero_tactile_parameter_anchor` 把触觉参数以零贡献挂回 autograd 图（`n0_twam/models/model.py:L1522-1537`；`n0_twam/train.py:L607-609`），否则 FSDP 会看到 None grad 与 bf16 grad 混用。
- **论文未提的 `ContactStateGate`**：用（预测的）GlobalTactile hidden 对 action 流做 zero-init FiLM 调制（`n0_twam/models/model.py:L845-902, L1912-1914`），base 与 post-train 配置均默认关闭（`twam_base_cfg.py:L95`、`twam_posttrain_cfg.py:L121`）。
- **推理时预测触觉与观测触觉分工**：video pass 之后，action pass 条件在**生成的** GlobalTactile 上，而跨 chunk 的 KV cache 用**观测的**触觉重新编码以保持 grounding（`n0_twam/n0_twam_server.py:L1044-1062`）。
- **一个论文完全没有的效率数字**：内部闭环评测基线是 video 3 步 / action 4 步去噪（`n0_twam/configs/twam_posttrain_server_cfg.py:L32-36`，注释 "3/4 = the baseline all internal closed-loop scores used"），而通用 server 配置是 10/10（`twam_server_cfg.py:L42-52`）。
- **clean 历史重加噪的具体强度**：video 以 0.5 概率重加噪（`n0_twam/train.py:L382`）、action 为 0（`L389`）、tactile 由 config 给 0.5（`twam_base_cfg.py:L129`），重噪水平采自 `[0.5, 1.0]`（`n0_twam/train.py:L345-351`）。论文只说"轻微重加噪"。
- 其余硬编码：触觉 token 用 `tactile_pos_offset = 10000` 在 RoPE 上与视觉分离（`model.py:L1011`）；每段序列 pad 到 128 的倍数以适配 FlexAttention（`model.py:L1712`）；tactile 专家不参与 text cross-attention（`model.py:L268`）；MoT 强制层粒度 activation checkpointing，注释称否则 80GB 也会 OOM（`mot.py:L255-260`）。

**复现路径**：依赖 torch 2.9.0 / diffusers 0.36.0 / numpy 1.26.4，Python >=3.10,<3.13，lerobot 0.3.3 必须 `--no-deps` 单装（`requirements.txt:L1-8, L23-28`）。权重取自 HF `NeoteAI/n0-twam-base`（`README.md:L110-114`）；数据走 LeRobot v2.1 → `script/encode_lerobot_n0_latents.py`、`script/encode_tactile_latent.py`、`script/build_task_pool.py` 离线编成 latent，训练入口 `run_posttrain.sh`（torchrun，默认 `NGPU=8`），服务入口 `n0_twam/n0_twam_server.py`，serve 需 ≥40 GB 显存（transformer ~14 GB + VAE ~3 GB + umT5 ~11 GB，`docs/DEPLOY.md:L48-52`）。预训练管线与预训练数据未发布（`README.md:L20`），因此该版本可复现的是 post-training 与闭环推理，不足以复现论文主结果。
