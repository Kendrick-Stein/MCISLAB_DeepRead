...
- **2026-09-18｜08-11 记下的"监督应经共享表示塑形 policy、预测的未来不该作为 policy 输入"从四篇收敛降级为按任务域分裂的争议**：[[Papers/2609-OpenWAM]] 用同族 Wan2.2-5B 底座、同样的 joint flow matching co-train、同一组 mask 消融，在 RoboTwin2.0 上拿到相反方向——isolated 87.41、video-sees-action 87.63，而 action-sees-video 92.39、mutual 92.15，切断 world→action 通路掉约 5 pp（缺口 4.98 与 4.76 pp，原文一致性已核查），而 [[Papers/2608-SimWAM]] 在驾驶上测到的是 90.3≈90.2≈90.1。两篇的设定太接近，归因于实现差异不成立。最可能的解释变量是任务域：驾驶的相关状态基本由当前前视观测决定，接触密集的操作则必须条件于预测出的接触与物体动力学——这是假设，无人测过。可执行性是这条记录最有价值的部分：OpenWAM 以 Apache-2.0 释出覆盖每个消融臂的 46 个 checkpoint，把同一组 mask 消融跑到接触密集与非接触两组任务上即可判定。在此之前该结论必须带任务域限定引用。另需分清强弱两类证据——本条依据的是 source-verified 的近 5 pp 缺口，而该文另一条"最优 mask 在预训练前后改号"的 finding 建立在 +0.16/+0.70/+0.72 pp 三个单次运行差上、与其自身表间不调和量级（0.03–0.20 pp）同阶，两者不可混用（[[Topics/WorldModel-Survey]] 路线 4 / Takeaway 33）
- **2026-09-18｜world model 评测器本身成为被测对象，而目前唯一可审计的那个，其高一致性是用压低弃权率换来的**：[[Papers/2608-HarnessEvalW]] 把评测写成 agentic harness（18 模型 × 330 case，3 轴 8 setting），与 5,000 条人类 A/B 的 Bradley–Terry 排序达 ρ=0.93（Intentional）/ 0.87（Physical），稳定性包络 0.33 对 WBench 的 1.61 BT 单位。但同视频同 judge 的对照里，Physical 准确率 31.9%→71.7% 的同时 draw 率从 52.2% 掉到 1.8%，增益主要来自决断子集的重归一而非判别力提升（该分解系按原文两数反推，论文未作）。这是 09-07 记下的 WorldSimProbe judge 修正的下一步：那次修的是 judge 的规则，这次把 judge 的可信度整个变成了实验变量。边界：人类对照只覆盖 8 个 setting 中的 2 个、18 个模型中的 9 个，未报标注人数与 IAA；Overall 是 330 case 的无权重算术平均，而无权重聚合正是 [[Papers/2608-WorldExam]] 明确拒绝的做法（[[Topics/WorldModel-Survey]] 路线 10d）
- **2026-09-18｜video world model 的条件化大量挂在表观相关而非物理转移上，且三类常规干预都修不动**：[[Papers/2608-PAWBench]] 把对齐拆成 support 与 probability-mass 两层，50 场景 / K=50 rollout 下测到 31.2 的失准量级，匹配的 Monte Carlo 基线为 8.33（99% 分位 < 9.22），采样噪声解释被排除；失效呈方向性——causal underreact 与 non-causal overreact。更要紧的是干预侧：oracle prompt 只实现 37.6–58.1%，C2C 微调把 Coverage 从 71.7% 抬到 74.8%、Wan2.2 从 63.4 抬到 69.2，5 个 LoRA 无一同时改善两层。这把 08-02 记下的"生成保真 ⇒ 懂物理被同一模型内的指标背离反驳"从指标背离推进到干预抵抗：不只是现在不懂，而是提示、微调、适配器三条常规路子都没打动它。边界：判定链依赖 Gemini 3.5 Flash，正属其自证失准的那一族 VLM；81.3% 人类一致率只在 888/1,500 的明确子集上成立；均值只在过 readout gate 的场景上可比。与 [[Papers/2608-WorldSimProbe]] 同向，但二者均为库内单篇证据、非独立复现（[[Topics/WorldModel-Survey]] 路线 10c）
- **2026-09-18｜07-29 记下的 executable structure 这一支长成独立路线，两个方向相反的实例都已到位，但两篇验证的都不是各自论证的那个命题**：[[Papers/2608-CodeAsWorld]] 走反解（video → MuJoCo `scene.json` + SDK，propose–instantiate–execute–render–verify，K=5），[[Papers/2609-ProgrammableWM]] 走正驱（canonical state → 确定性 state compiler → dense 控制图 → 冻结底座）。两边的验收都落空：前者在 QuantiPhy 159 题上 9B 得 55.4 对 Gemini-3.1 Flash 54.8（+0.6，无误差棒），而 discovery loop 的收益从未连到这个分数，主文的 discovery 分析用的还是 animation engine；后者 CombatStateBench Count 94.00 / State 98.00 远超基线，但 dense 像素对齐条件已把答案写进输入，测的是条件遵循而非 programmability，且全文零 ablation、缺同底座不挂 ControlNet 的对照行。分母须一并记住：State 只摊在 50 次死亡事件上（最小刻度 2 pp），"98 对 58" 实为 49 对 29 个事件。缺的实验对两篇是同一个——程序 vs latent vs pixel 的表示对照（[[Topics/WorldModel-Survey]] 路线 11）
