...
从表格数据（CSV / DataFrame / 内联表格）与自然语言图表描述出发，产出可复现的 matplotlib 脚本与出版级 PNG 图表，供论文正文使用。输入是用户提供的数据源与图表描述；输出是可重复运行的 `plot.py`、对应的 `plot.png`，以及 `figure-spec.md` / `audit.md` / `final-status.md` 三个校验产物。默认写入用户指定的论文项目目录（未指定则写入当前工作目录），不写入 vault 内部目录。
...
- Write the script.
- Execute it with `python3 plot.py`（ReadPaperMachine 未使用 uv-managed venv，直接用当前工作区的 Python 解释器即可；若用户自己的论文项目配置了 `uv`，也可以用 `uv run python plot.py` 代替）。
...
```bash
python3 skills/4-writing/paper-figures/scripts/validate_figure.py --output-dir <output-dir> --spec <output-dir>/figure-spec.md
```
...
- 图表数据只能来自用户提供的结果文件（CSV/DataFrame/内联表格）或 `Experiments/` 中记录的实验结果，不虚构或外推未经验证的数据点
- 输出（`plot.py` / `plot.png` / `figure-spec.md` / `audit.md` / `final-status.md`）写入用户指定的论文项目目录（未指定则写入当前工作目录），不得写入 vault 内的 `Papers/`、`DomainMaps/` 等目录
- 脚本运行失败或审计未通过时，如实报告 `FAILED_NEEDS_HANDOFF` 并说明具体阻塞原因，不得静默降级为"脚本能跑就算通过"
- 不添加描述未要求的视觉元素（回归线、参考线、额外图例、聚合统计等），也不得静默丢弃数据点或用坐标轴范围裁掉描述中点名的区域
- 一次交付一张 PNG、一次 `savefig` 调用；不产出交互式仪表盘（Plotly/Streamlit/Jupyter）或非数据类的流程图/示意图

## Verify

- [ ] `plot.png` 已生成且文件非空（`scripts/validate_figure.py` 可做结构性校验）
- [ ] 坐标轴标签、单位、刻度、图例、标题均与 `figure-spec.md` 一致，字号/DPI（≥300）/配色符合 `references/publication-style.md`
- [ ] `audit.md` 每一项 pass/fail 均已填写，`final_status` 诚实反映结果（有未通过项时不得标记 `PASSED`）
- [ ] 源数据路径（CSV/DataFrame 来源）已记录在 `figure-spec.md` 的 `data_sources` 字段
- [ ] 输出目录符合用户指定路径，未污染 vault 内部目录
