...
| [[Papers/2607-QwenUIAgent]] | 生产规模真机集群 | 100+ 台物理设备 / 150+ app，health-aware scheduler + virtual display 并发（自报总 rollout 吞吐约 20×） | 自建 MobileWorld-Real 由自建 AutoJudge 评分且 env_error 不入分母；无隔离"真机训练"贡献的受控 ablation |
| [[Papers/2608-AppDeltaWorld]] | GUI world model 作环境替代 | transition-grounded 检索 + delta code + diffusion 渲染；闭环 rollout 产出 33,133 条合成轨迹 | 功能逻辑分低于图像基线；合成数据单独训练掉点，须混真实监督；consensus reward 不成立 |
...
真机一侧此前只有单点证据，[[Papers/2607-QwenUIAgent]] 把它推到可调度资源池的形态：100+ 台物理设备与 150+ app 由 health-aware scheduler 按设备、应用、账号、网络与显示的健康度租借并在失败时改路，不健康目标进动态黑名单待人工修复与复验，virtual display 使单机并发承载多个 app session（自报把集群总 rollout 吞吐提升约 20×）。它也暴露了真机供给特有的记账问题——"模型失败"与"环境失败"必须先分开才能计分，该系统用 VLM judge 对完整轨迹做 success / model failure / environment failure 三分类，并把确认的环境故障回流给 scheduler。这条路线的证据边界必须与其规模一并引用：主力数字 MobileWorld-Real 92.2% 建在作者自建的 409 任务 / 104 app benchmark 与自建 AutoJudge（5 个 VLM judge 多数投票、env_error 不计入分母、与人工标注一致率 92.8%）之上，其对次优系统 3.5pp 的领先小于该 judge 自身 7.2pp 的不一致率；且全文没有"同一模型加/不加真机数据"的受控对照。因此现有证据支持的是"真机供给可以被工程化到生产规模"，而非"真机训练是收益来源"。这与 [[Papers/2607-HyMobileAgent]] 构成互补：后者量化了 sim-to-real 缺口有多大（AndroidWorld 82.6% → 私有真机 42.0%），前者给出直接在真机侧建供给的工程形态，但两者都尚未把真机数据的因果贡献从"更多数据"中分离出来。

[[Papers/2608-AppDeltaWorld]] 把供给谱系推到第四种形态——不搭环境而学环境，并给 GUI world model 补上此前缺失的 transition 合法性结构：app 级 action-transition index（click 落点量化到 6×12 网格）约束"当前屏加这个 action 能到达哪些页面"，检索 Level-1 HTML 骨架后生成 Level-2 delta code、diffusion 填图片槽，找不到受支持的 target cluster 即判 invalid——"拒绝非法 action"由可查表结构而非生成模型的隐式知识承载。它同时是"保真度排名与下游收益排名不同构"（[[Papers/2608-WorldProxy]] 的验收论点）的一个双口径实例：CMGUIBench 总分 73.51 居首，但功能逻辑分 S_ad/S_id（79.69/77.00）低于图像基线 GPT-Image-2（91.73/83.40），下游 MobileGym 14.1% 也落后三个同量级开源模型。对环境替代路线更有解释力的是它的两条机制性结果：逐 action-type 拆解显示合成经验的增益在动作类型选择与停止时机（wait 14.19→72.97、click 仅 91.82→95.88）而非坐标 grounding——低保真渲染器对决策层经验仍然有效；而 ADW-only 训练相对基座反而掉点、必须与真实数据混合，说明合成经验的多样性天花板由 world model 的训练分布决定，恰好绕不开"敏感 app 缺真实轨迹"这一原始动机所在的分布区域。其 consensus-reward 负结果（8 rollout 聚类，23.5% 的 group 无唯一赢家）还表明这类 world model 缺少状态等价判定，test-time verifier 用法目前不成立。
...
与视觉/结构侧的表示重构平行，web agent 有一条独立成型的 **observation reduction** 线，针对 raw DOM/HTML 常达 10k–100k token 的问题优化喂给 agent 的观察。四条路线已固化：程序化剪枝（[[Papers/2511-Prune4Web]]，候选削减 25–50×，low-level grounding 46.8→88.28）、LLM 选行检索（[[Papers/2510-FocusAgent]]，削减 >50%）、规则式结构重构（[[Papers/2605-A11yCompressor]]，OSWorld input token 压到约 22% 的同时 success +5.1pp）、与"缩短"正交的表示对齐（[[Papers/2410-AgentOccam]]）。这条线最有价值的产出是三个跨论文的校正性发现：其一，**优化 ≠ 省 token**——AgentOccam 每步观察 token 反而从 vanilla 的 2210 升到 2930，真正起作用的机制是"对齐 LLM 预训练分布 + 降噪"而非缩短长度；其二，**压缩并非普遍有益、且高度依赖底座**——[[Papers/2604-ReadMoreThinkMore]] 显示强模型（gpt-5.1、claude-sonnet-4-6）用完整 HTML 反而 +14.6~17.5pp、弱开源模型用 HTML 大幅退化（gpt-oss-20b −18.8pp）；其三，**收益随模型变强而蒸发**——Prune4Web 对 GPT-4o 零提升、FocusAgent 在 WebArena 反低于全观察（32.3 vs 36.5）。该子领域已成熟到自建廉价评测代理（[[Papers/2605-MFSCoverage]]），侧面印证方法层面接近饱和。这条线的方法都在内容与长度轴上操作；[[Papers/2409-ElementOrdering]] 补上第三个轴——元素**呈现顺序**的扰动伤害与删除全部可见文本相当（GPT-4V 74.07→44.44，详见 §6.7.2），改变顺序的重构方案应报告顺序保持性。同一条张力在训练语料侧也成立：[[Papers/2605-Weasel]] 的 target-centered pruning 只保留 ground-truth 动作目标节点周围的连续窗口，在约 32% token 预算下是四种剪枝里唯一不低于未剪枝基线的（10.9 对 10.3，semantic 9.1、prune-by-bid 8.5、两者叠加 7.3），把窗口从目标节点向外偏移则成功率近似线性下降；但把它单独施加在全量训练语料上，在最强底座上反而是全表最差的一行（Qwen3-8B MiniWob 59.4→40.3、WorkArena L1 33.3→15.5），只有与步骤选择组合后才恢复，该交互原文未作解释（详见 §5.10）。削减是否有益取决于底座与它在流水线中的位置，而不取决于削减率本身。

上述路线都在 element-level 表示内决定"留哪些内容"；[[Papers/2605-Region4Web]] 质疑的是更上游的默认——observation space 沿用了 action space 的 element 粒度，而这只是被所有 harness 默认继承的实现细节，并非必然。它用约 536K 参数的边分类器把 AXTree 自底向上切成 functional region（一组服务同一功能目的的元素），0.6B 模型为每个 region 生成跨步稳定的 purpose 与随步变化的 state summary；进入新页面时由 actor backbone 选出任务相关 region——选中的暴露完整子树以保住动作所需的 element 粒度，未选中的只留一句 purpose——页内则以增量 diff 维持这份 digest 而非逐步重建。WebArena 四个 backbone 平均观察 6,437→3,671 token（-43%）、成功率 +2.3pp；唯一近似等长的对照中 element-level 变体 46.1 低于 region-level 的 53.9（长度相当）。两条设计断言比数字更可迁移：观察单位与动作单位可以不同构；观察中跨步稳定的功能成分与易变的状态成分应有不同的缓存周期。同样值得记录的是它暴露的共性缺口：没有把 full-observation baseline 压到同等 token 预算的对照，因此"更短且更好"与"长上下文 distraction 代价"无法归因分离——这正是本小节 AgentOccam / ReadMoreThinkMore 两条校正性发现所指的同一审计维度；且域级回退集中在 GitLab / CMS 这类高密度、弱语义分组界面（AgentOccam 配置下 GitLab 66.7→50.0），提示 region 粒度的适用面本身是站点动作空间结构化程度的函数。
...
#### 4.5.1 开源 Web Agent Harness 与 Observation 空间

开源 Web Agent 框架并不共享一种统一的 harness 抽象。本节用四种接口与状态组织方式归纳所核查的七个框架：以 browser session 为状态的 autonomous step loop，允许开发者在 code 与 natural-language primitive 之间切换的 hybrid SDK，把浏览器当作可丢弃执行环境的 code-as-action，以及将 task/reset/step/validate 标准化的 research harness。这是分析框架而非官方、互斥或穷尽的 taxonomy，部分系统跨越多类。因此，“observation space”不能只写成 DOM 或 screenshot；还必须说明它由 harness 每步自动注入、由 agent 按需调用，还是由 agent 自己编写程序探测。

```mermaid
flowchart LR
  G["Task goal"]
  subgraph A["Autonomous step loop"]
    A1["Structured / visual observation"] --> A2["LLM predicts next action"] --> A3["Persistent browser session"] --> A1
  end
  subgraph H["Hybrid SDK"]
    H1["Code or natural-language call"] --> H2["act / extract / agent"] --> H3["Browser session + cache"]
  end
  subgraph C["Code-as-action"]
    C1["Write program"] --> C2["Run disposable browser"] --> C3["Inspect logs / screenshots"] --> C1
  end
  subgraph R["Research harness"]
    R1["setup / reset"] --> R2["obs / action / reward"] --> R3["validate + trace"] --> R1
  end
  G --> A1
  G --> H1
  G --> C1
...
*四种 harness 的核心差异是状态归属与 observation 调度方式：浏览器状态、缓存动作、本地 workspace 与 benchmark episode 分别是四类系统的主要持久化对象。*

下表只比较公开文档或源码中能核实的接口。对 Skyvern 一类未把“每步给模型的完整 observation object”当作稳定 public API 的系统，表中保留为“未文档化”，不从“vision-first”等定位反推 screenshot-only 实现。

| 框架 | Harness 范式与状态归属 | 执行循环 | Agent-facing observation | Action / verification | 设计边界 |
|:--|:--|:--|:--|:--|:--|
| [browser-use](https://github.com/browser-use/browser-use) | Autonomous step loop；状态属于持久 `BrowserSession` 与 agent history | step start 取 browser state → LLM 产生一批可配置数量的 action → 页面变化时停止当前 batch → 重新观测；支持 before/after-step hook | 核心是经 paint-order 过滤的可交互 DOM/AX 节点、元素 index、URL/title/tabs；screenshot/vision 是否加入可配置 | CDP/browser tools 执行 index/coordinate/keyboard/navigation；action error 进 history，可选 LLM trace judge 在结束后评估轨迹，但无 benchmark-level `validate()` 或后端 state diff | 对通用自主浏览最直接；element map 是短期快照，页面更新后必须重建；默认成功判断仍主要由 agent 完成 |
| [[Papers/2412-BrowserGymAgentLab|BrowserGym + AgentLab]] | Research/evaluation harness；状态属于 Gym episode，AgentLab 另管理 study/trace | task `setup()` / env `reset()` → `obs` → `step(action)` → task `validate()` → reward/done；AgentLab 并行调度多 benchmark/config/seed | 可配置 observation dict：task chat、tabs URL/title/active index、screenshot、DOM、AXTree、`bid`、bbox/visibility/SoM、focused element、last action/error | High-level action mapping 或 raw Python/Playwright；验证委托给各 benchmark 的 `validate()`，AgentXRay 保留逐步 trace | 最适合受控比较 observation/action 设计；统一调用层不等于统一 verifier，也不把 hidden backend state 暴露给 agent |
| [Stagehand](https://docs.stagehand.dev/v3/basics/agent) | Hybrid code + AI SDK；状态属于 browser session，workflow 可缓存已解析动作 | 开发者在确定性 code、`act()` / `extract()` / `observe()` 与多步 `agent()` 之间选择；`observe()` 先返回候选动作再执行 | 三个 mode 定义的是可用 tool set：DOM 提供 semantic tools，并可按需调用 `ariaTree` / `screenshot`；Hybrid 另加 coordinate tools；CUA 是 provider screenshot loop。`observe()` 输出 selector/method/args 候选，不是完整 RL observation | DOM semantic actions、coordinate actions、structured extraction；支持 cache/self-healing 和 action/result trace，但 `completed` 只是 agent belief，不是应用后端 verifier | 适合“已知流程用 code，未知页面用 AI”的生产自动化；三种 mode 的分数与失败不能在不报 mode 时混比 |
| [Skyvern](https://github.com/Skyvern-AI/skyvern) | Workflow/RPA platform；task/workflow、browser session 和 server-side run 状态持久化 | 多 agent 规划/执行高层 task；同时提供 Playwright-compatible `page.act/extract/validate/prompt` 与 workflow blocks | 官方定位是 Vision LLM 理解当前页面，但未将完整 per-step model observation 定义为稳定 public object；structured extraction 是公开输出面，livestream 服务于人类观测与调试 | AI page actions、表单、文件、login、HTTP/custom-code block；`validate(prompt)` 返回页面判定 | 适合跨站业务 workflow，但 observation 细节不适合当作可控学术变量；`validate(prompt)` 不等于确定性业务 state verifier |
| [Magnitude](https://github.com/magnitudedev/browser-agent) | Vision-first controllable automation/test harness；状态以 browser run 与 test artifact 为主 | 开发者用高层或低层 `agent.act()` 组合 flow，另用 `extract()` 与 test runner 验证 | navigation/interaction 依赖 screenshot 与 VLM 像素坐标；structured extraction 另读 DOM content，因而是“视觉操作 + DOM 抽取”双路径，不是纯 screenshot 全栈 | 鼠标/键盘坐标动作、schema extraction、visual assertions/test runner | 绕开 DOM selector 对复杂视觉界面有利，但强依赖大型 visual-grounding model；学术评测仍需另接 reset/reward harness |
| [Webwright](https://github.com/microsoft/Webwright) | Code-as-action；状态归属本地 workspace（code/logs/screenshots），浏览器可丢弃 | coding agent 写 Playwright Python → 执行 → 按需看 page state/screenshot/log → 修复脚本；最终产物是可重跑程序 | 没有固定每步 observation tensor/object；terminal output、agent 主动查询的 DOM/page state、保存的 screenshot 和 run artifact 构成按需观测 | Free-form Python/Playwright；通过脚本重跑、仪器化 screenshot 和视觉 self-check 验证 | 适合长程、可重复 workflow；动作空间和权限比离散 browser tools 更大，安全、sandbox 和 task verifier 由 host 负责 |
| [LaVague](https://github.com/lavague-ai/LaVague) | 早期模块化 autonomous loop；状态属于 WebAgent/browser driver | World Model 读 objective + current page 生成 instruction，Action Engine 再编译为 Selenium/Playwright code 执行 | Driver 向 World Model 提供 screenshot + HTML source，WebAgent 另维护 short-term memory / past actions 与 interaction zone；未给出跨 driver 的 typed observation contract | instruction-to-code Action Engine；轨迹记录 action code、observation method、bbox/viewport 等 | 适合研究 planner/executor 解耦的历史设计；缺少 BrowserGym 式 benchmark contract 与 app-grounded verifier |

这七个框架的交集比它们各自的功能列表更有研究价值。第一，主流系统已经不再在 DOM 与 screenshot 之间二选一，而是在“固定 hybrid”、“按需 vision”和“由 agent 主动写 probe”之间选择 observation policy。第二，状态所有权改变了错误恢复形态：browser-session loop 通常通过重观测继续，Webwright 通过修复并重跑程序恢复，BrowserGym 通过 episode reset 恢复；这三者不能用同一个“memory”概念概括。第三，在本次核查的 public interface 中，只有 BrowserGym 将 task `validate()` 纳入环境协议并由 `env.step()` 返回 reward/done，但它对 agent 不可见；其他框架的 `validate()`、visual assertion、trace judge 或 self-check 多数仍是模型/页面层判断。在本轮查阅的官方文档所列默认 public agent-facing contracts 中，未发现单一接口同时文档化 app-level state diff、后端副作用报告与 full-state rollback；这一结论不排除未公开的托管能力。

这一结构也说明了 observation 设计的两个实证风险。首先，结构化通道可以更 token-efficient，但 [[Papers/2607-GUIStateBelief]] 已证明 stale structure 会在与截图冲突时主导模型信念；因此 element ID 或 selector 必须绑定 snapshot version/freshness，不能只追求“稳定 ID”。其次，按需 observation 减少了固定 context，但它同时把“什么时候看、看什么”变成策略能力；Webwright 与 browser-use `use_vision=auto` 的收益不能只按每步 token 数比较，还应同时报告额外 probe 数、wall-clock、capture race 与遗漏关键状态的比例。
...
工具通道是否无条件有益，目前最系统的受控检验来自 [[Papers/2604-ToolIllusion]]：三套 tool-use web framework（人工 REST API 的 Hybrid-Agent、LLM 合成 Python 函数的 SkillWeaver、刻意小工具集的 WALT）× 五个跨家族 backbone，在 WebArena / VisualWebArena 上同 framework 内只切换工具可用性。人工 API 使五个 backbone 全部获益（WebArena +12.4~+19.6pp），而 LLM 合成工具的收益是条件化的——仅当使用者明显弱于合成者时稳定成立，GPT-5 用 GPT-5-mini 合成的工具反而 52.9→50.9；固定使用者、更换合成者的反向扫描同样支持这一单向 capability-distillation 模式。工具库规模不构成价值：SkillWeaver 62% 的工具属高复杂度、79% 在整个评测中零调用；工具还有隐性税——Hybrid-Agent 开启工具后每站点 token 从 10.5M 涨到 33.9M、平均步数不降反升（7.1→8.3），真正决定上限的是功能覆盖与可组合性（其 52% 任务靠 ≥2 个低复杂度工具组合完成且成功率最高）。这组证据把 §4.7.1 表中"API 通道的收益"改写为三变量函数：工具来源（人工/合成）、合成者-使用者相对能力、覆盖与可组合性。边界：跨 framework 数值不可直接比（观测格式与预算未对齐，GPT-5 在 WALT 与 Hybrid-Agent 的无工具基线差 16.4pp）、单次运行无方差，且沙盒站点 DOM 稳定，合成工具在真实站点的选择器漂移税只会更高。[[Papers/2605-PlanThenExecuteWeb]] 从基础设施侧补上"需工程化构建 API"一格的量化下界：WebArena 的 Postmill 只暴露 16 个 REST API、仅能覆盖其 129 个任务的 33%——完整可信 API 这一假设与现实覆盖率之间的落差才是 API 通道的第一障碍（该文的安全论证见 §6.11.1）。
...
| 路由范式 | 机制 | 代表工作 | 关键数字 |
|:--|:--|:--|:--|
| RL 训练联合 policy | 同一 policy 在 system prompt 中同时看到 API 函数与 GUI 原语，由 RL 学会选择 | [[Papers/2508-ComputerRL]]、[[Papers/2607-QwenUIAgent]] | GUI-only 11.2%→API+GUI 26.2%（+134%），Office 域 6.2%→27.9%；后者不是消融而是使用分布：OSWorld-Verified / v2 上 CLI 占全部动作 40.7% / 55.1% |
...
配比与去重都作用在语料或轨迹层，[[Papers/2605-Weasel]] 把处理单位下沉到轨迹内部的单步，并把"哪些 step 进 SFT"写成一个可解的固定预算子集选择问题：unary 项用 BERTScore 度量该步状态与任务目标的语义相关度，pairwise 项取状态距离与 reasoning+action 拼接串距离的较大值来度量冗余，两项加权后贪心求解，主实验每条轨迹只留 3 步。在 AgentTrek 与 NNetNav 两个离线轨迹语料上，选出的 10K 步在三个 backbone 上都不劣于 52K–80K 的全量 SFT，训练时间压掉一个量级：Qwen2.5-7B 的 WebArena-Lite 14.5 对全量 10.9、未训练 base 5.5，加速 11.3×；Qwen3-8B 21.2 对 17.7、base 16.4，加速 10.7×（自建 self-hosted 环境上的零样本迁移评测，WebArena-Lite 由 GPT-4.1-mini 判定、WebArena 由 GPT-4-1106-preview 判定）。目标函数的消融比主结果更有信息量：只用 unary 项 10.9、只用 diversity 项 7.9、两者合用 14.5，说明相关性与覆盖度是互补而非替代的信号；而 diversity 的价值几乎全部来自动作侧——reasoning-only 13.9 对 state-only 9.7——即 web 轨迹里的冗余更多表现为重复的动作模式而非重复的页面外观，这与 [[Papers/2601-EvoCUA]] 按"重复且无进展"删步时同样偏动作侧的判据同向。

这条路线有两处边界。第一，该目标优化的是目标相关性与非冗余，不判断某一步是否正确：一个错误但语义贴近目标、且与已选步骤不重复的动作会被选中而非被过滤；其前提是专家轨迹整体可信（AgentTrek 由教程合成、NNetNav 有回溯标注），越出这个前提它不提供 step-level credit assignment。第二，增益的解释力受两处结果限制——在最强的 Qwen3-8B 上，全量 SFT 本身就把 MiniWob 从未训练时的 61.1 拉到 59.4、WorkArena L1 从 35.2 拉到 33.3，Weasel 相对未训练 base 的净增益只有 +0.8 / +3.6，其主要成就是让离线 SFT 不再有害；同一 backbone 上 Pruning+LLM-Judge 拿到 WebArena-Lite 19.4、Weasel 21.2，而去掉 reasoning synthesis 组件后的 Weasel 只有 17.0，因此"选择目标本身优于 LLM-judge 选择"在该 backbone 上并不成立。
...
Planning 的时间轴此前几乎不作为变量出现。[[Papers/2607-AAPT]] 把它显式化：当界面按环境的时钟开合动作窗口（transient dialog、boot prompt、短时效认证、反应类游戏状态），autoregressive decoding 本身就占据了 decision-time critical path，动作算对了而窗口已关。其做法是在空闲期用同一个 frozen 模型预编译一棵**扁平**（非递归）的 contingency policy tree，节点携带预测状态、observable guard、pre-authorized 动作、per-branch deadline、置信度、风险等级与 fallback；事件发生时只做一次低 token 的 observer 调用把当前帧路由到某个分支，critical path 上不再生成，判据也随之改写为 `success = 正确动作 ∧ 动作在 deadline 之前`。该文最强的一击是排除了"提早行动就够了"这个自然的替代解释：open-loop 与 predict-and-replan 两条对照臂在竞争窗口下均为 0/42，实测 per-decision path 反而更长（observer 273 ms/30 tokens vs reactive 411 ms/48、predict-and-replan 484 ms/66）——anticipation 只有在把生成挪出 critical path 时才转化为延迟收益。可迁移的也不是这套机制（作者自陈其单个组件均不新颖），而是一条从独立测量量预测收益出现位置的规则：设 planner 端到端延迟的 95 分位为 $L_{p95}$、安全裕度为 $M$、树保持有效的墙钟区间为 $T_{cover}$，则要求 $T_{cover} \ge L_{p95}+M$；prep-budget sweep 的 crossover 实测落在 $L_{p95}+M \approx 2.4$ s（prep 0.5–1 s 时 0/42、2 s 打平、4 s 时 42/42），与规则预测一致。

这条结果的外部效度必须与之一并引用。主证据链跑在作者自建的三键 `key_prompt` 计时 benchmark 上（650 ms 窗口 0.79 vs reactive 0.50，per-seed 配对 exact McNemar p=1.8e-3，端点预注册），理由是 OSWorld / WebArena / Mind2Web / AndroidWorld 均不提供隔离 deadline-sensitive 动作选择所需的毫秒级控制；而唯一的外部 benchmark DynaCU-Bench 上是平局（reactive 7/39 vs AAPT 6/39，p=1.0）。成立条件同样很硬：需要 $\ge L_{p95}+M$ 的静默期，需要解码足够快的模型（两个 dense 32B 模型 decode >1 s，AAPT 反而远劣于 reactive——EvoCUA-32B 0/43 对 22/43），且 guard 与动作必须在事件前可绑定，late-revealed 坐标与多步序列直接违约。分支预算 $K_{max}$ 还是一个手工设定的**正确性**参数而非性能参数：设小了 planner 写出泛化 guard，设大了用带默认动作的 catch-all 填满剩余槽位，两侧都把"错过"转成"按错键"（k=1 时 13/42、k=2 时 12/42、k=7 时 15/42，匹配预算 k=5 时 0/42）——而"下一步有几个可能结果"在真实 GUI 里恰恰是事先不知道的量。因此本节采纳的是它对 planning contract 的补充，而非把 anticipatory 预编译当作长程 planning 的一般方案。

Planning 的核心对象正在从"下一步 thought"转为可检查的状态转换契约：当前 subgoal、前置条件、预期 UI 变化、完成证据、可逆性、失败后的替代路径，以及 [[Papers/2607-AAPT]] 补上的时间字段——该转换的有效窗口与决策 deadline。缺少这些字段时，长 CoT 只是更长的不可审计内部状态。
...
[[Papers/2605-PlanThenExecuteWeb]] 把表中 trust isolation 一行从运行时机制上移到架构层。这篇 position paper 的论证是：ReAct 的缺陷是结构性的——网页上不可信内容是常规 observation stream 的组成部分而非例外通道，攻击者控制的 token 恰好出现在模型决定下一动作的同一次 inference 里（confused deputy），因此 model-centric 防御与 runtime monitor 都不触及要害；plan-then-execute 用控制流预先固定的程序把不可信内容限制在数据流内——可填参数、影响既有分支走向，但不能增删步骤、选择新工具或触发 replanning——并使"在观察任何不可信内容之前做静态安全审计"成为可能。其经验支撑是对 WebArena 860 个任务的人工静态分类：100% 可用 PTE 表达、81.28% 连 runtime LLM subroutine 都不需要，站点方差极大（Wikipedia 8.70% vs GitLab 90.69%）——web 任务需要多少反应性不是常数，而是站点动作空间结构化程度的函数。三条边界必须同读：这是可表达性上界而非端到端能力，全文没有运行任何 agent、没有 success/token 数字；"assume complete and trusted APIs" 这一前提吞掉了核心困难（Postmill 实测见 §4.7.2）；safe-with-influence 类任务仍暴露于 outcome manipulation，而预处理期用 LLM 自动生成站点 SDK 会把注入面从 runtime 搬到 build time 而非消除。与 [[Papers/2607-UCM]] 的 privileged planner / quarantined content 属同一设计家族，区别在于 PTE 把隔离做成程序语义而非运行时信任标签。
...
OS-ATLAS 在 grounding pre-training 后使用跨数据集 action fine-tuning，将平台异构动作统一为共享语义。[[Papers/2410-OSAtlas]] GUI-Libra 进一步混合 reasoning-then-action 与 direct-action 数据，并提高 action/grounding token 的训练权重，以缓解长 CoT 对 grounding 的干扰；这是某工作的条件性发现，不代表 reasoning 普遍有害。[[Papers/2602-GUILibra]] EvoCUA 则把 cold-start SFT 限定为行为先验注入，随后才由 rejection sampling 与 preference optimization 消费 agent 自身产生的经验。[[Papers/2601-EvoCUA]] 监督目标还有一个数据侧的自由度：[[Papers/2605-Weasel]] 表明 SFT 的 loss 不必铺满整条专家轨迹，按目标相关性与非冗余选出的约 19% 步骤即可在三个 backbone 上不劣于全量训练，而底座越强、全量离线 SFT 相对未训练 base 的净收益越接近零（详见 §5.10）。
...
[[Papers/2608-UIMate]] 从反面给 validator coverage 补上一个量化边界。执行不变量 `R(E_0)=0, R(E*)=1`——初始状态得 0、参考完成态得 1——是这条 verifiable task 合成路线的标准收敛检查，UI-Mate 指出它只建立内部一致性：参考完成态与 reward 函数可以共享同一个语义盲点，两边一起错时不变量照样成立。对已通过该筛选的 evaluator 做审计后，仍约 18% 与指令语义不对齐，其中过严匹配占 40%、语义空洞的断言占 23%、判错目标占 19%。这与 SCALECUA 的 82.0%/58.3% task validity 是两层不同的失效：后者问任务本身是否成立，前者问任务成立且不变量通过之后 reward 是否真的在测指令要求的东西。方向上过严匹配占最大一类，与 [[Papers/2607-MisScoreCUA]] 在评测侧观察到的 programmatic oracle 偏严一致，但两者一个审计训练期合成 verifier、一个审计已发布 benchmark 的 evaluator，不构成互相复现。同一批消融另有两条对 RL 配方本身的阴性结论：adaptive curriculum sampling 与 process credit model 不一致地提升最终成功率，收益是常在不到一半的优化更新内达到相当性能；historical reasoning 仅在评测时开启可得 +3.43pp（SFT 模型）与 +2.27pp（RL 模型），纳入 RL 训练则出现 entropy collapse 特征并降低评测性能。上述数字均为作者自报，审计的样本量与人工校准未报，库内暂无独立验证。
...
[[Papers/2608-ROPSD]] 把 self-training 推到 test-time，并让 verifier 不止做选择：grounding 模型部署后在无标注界面上探索，一个在 GroundCUA 上以 GRPO 训出、held-out 二分类 accuracy 89.5% 的 MLLM Reflector 评估每次预测并生成文本反思，反思被条件化进 policy 自身构成 self-teacher，其与无条件 policy 的逐坐标 token log-ratio 作为 advantage 写回参数，把标量 reward 无法利用的自由文本反思转成稠密 token 级监督。Qwen2.5-VL-3B 在六个 grounding benchmark（ScreenSpot 系列 / MMBench-GUI / OSWorld-G，指标 Element Accuracy、adaptation 数据取自 benchmark 自身去标签）上 adapt-on-MMG 后平均 50.2→57.6（+7.4），同设置下 scalar-reward 的 GUI-RCPO 反而负迁移（−0.3）；去掉反思、或去掉抑制错误前缀污染的 Contrastive Calibration（把 advantage 分母换成被误导为“预测正确”的 inverse-prompted student）都直接 policy collapse。它把 VAGEN 悬置的问题——长期 self-training 会否放大 verifier bias——落成一个具体实例：整个闭环压在 Reflector 上，而“无需标注”只对 test-time 成立（Reflector 本身用带 GT 的 GroundCUA 训练），论文只给单一 Reflector 工作点、未刻画其准确率降到多少框架失效；且所谓 test-time adaptation 实为 transductive/离线设置（adaptation 数据即 benchmark 去标签、训练后再评测），并非流式逐样本 TTA，增益也仅在单步 grounding 上验证、未传导到 agent 端到端成功率。这条 test-time self-distillation 与训练期的 privileged-info 蒸馏（§7.13）共用“参数共享、条件化特权信息的 self-teacher”机制。库内暂无独立验证。
...
把生产 harness 本身当演化对象的一个具体系统是 [[Papers/2608-Ouroboros]]：一个 coding agent 通过 reviewed-commit pipeline 持续改写自己的 tools、prompts、context 组装乃至核心实现，改后的代码直接成为后续工作的 runtime，用一个在所有 context mode 下都 blocking 的多模型 diff-review 面板、加受保护 constitution（P0–P4 为不可删改的 protected core）充当上文所要求的"独立、可追溯且难以被当前 policy 操纵的 gate"，并以 launcher/repo 进程边界、不可经普通工具调高的 spend cap 与 operator 的 /panic 兜底运营安全。frozen seed、evolution off 下报 OSWorld-Verified 90.69%（327.39/361，Opus 5）、Terminal-Bench 2.1 audited 86.74%，并附 161 天真实部署（1,085 次 self-commit、94.2% agent-authored、近期 63.5% 被 review 阻断）。但证据边界压在 causal attribution 上：没有 day-0 vs day-161 的同种子对照，1,085 次 self-commit 与 benchmark 分数之间的因果链未被实验建立；多数 SOTA margin 落在作者自陈的 ±1.7pp binomial SE 量级内（OSWorld 90.69 vs leaderboard 首位 90.19 差 0.5pp 无显著性检验、SWE-bench Pro 与 Codex McNemar p=0.40、GAIA 持平），review quorum 的构成未披露，且为单一 lineage。因此它支持的是"harness 可作为演化对象、并配可阻断的独立 gate 长期运行"，不支持"self-development 因果导致了这些分数"。库内暂无独立验证。

[[Papers/2608-DarwinX]] 同样冻结 base model 只演化 harness，但把改进过程组织成 population selection 而非单条 lineage 的连续自我改写，因而给这一行补上另一种 gate 形态：准入不靠 review 面板，而靠一份 preserve-and-extend contract——子代必须净增益为正且回归有界，判定全部来自 benchmark 自带 verifier 的 avg@k，archive 保留所有变体（含全局更弱者）供跨 lineage 重组。四个 benchmark 按演化信号与测试集的分离程度排成梯子。Terminal-Bench 2.1 上冻结 GPT-5.5，base 由 75.5% 升至 83.2% avg@5，但该对照的 inference effort 未对齐（base 行为 default、DarwinX 行为 high）；换到 GPT-5.6 Sol / medium 得 84.7%，与 Claude Code + Fable 5 的 83.8%（xhigh）相差 0.9 点且落在两行各自 ±1.2 的误差棒内，原文措辞为 matches or exceeds。TerminalWorld 的 41 道 held-out 从 25 题升到 28 题（61.0%→68.3%），对最强现成 agent 的配对 McNemar p=1.0、对 base 为 p=0.45，附录另承认一个独立 skill-bundle 的 pre-TW 参考 harness 同样达到 28/41。TB2.1 上演化出的 harness 零改动迁到 SWE-bench Verified 得 421/500（84.2%），高出 80.8% 的 fix-skill 参考 3.4 点。四个 delta 中最大的那个需要单独读：WebArena-Infinity 的 1,260 道真实任务上 audit-clean pass@1 从 43.5% 升到 93.0%，同模型的标准 Browser Use 基线为 86.1%，但 base harness 在分应用表里多处远低于该基线（Elation prescriptions 20.0 对 90.8、Gmail 25.0 对 85.0），因此 +49.5 中有多少来自修复一个偏弱的起点、多少来自 harness 演化的净收益无法分离。

机制侧的自陈进一步限定了可引用范围。archive、parent selector、recombination operator 与 inference effort 均未单独随机化，算子级贡献只是 plausible 而非 causal。更关键的是，收益最大的那次 run 里 recombination 反复尝试而每次 merge 都被 revert（gate 共保留 26 次迭代、revert 36 次），增益实际沿一条短的主 lineage 累积——population 与重组这一核心主张恰恰在最强证据所在的 benchmark 上没有生效，只在 TerminalWorld 上有四个 specialist（各解 24/25/26/27 题）合并到 28 题的一次正面记录。TerminalWorld 的训练子集分数从 0.505 饱和到 1.000 而 held-out 只有 68.3%，31.7 点的 proxy 落差是本节表中 workflow/harness 一行标注的 benchmark overfitting 在搜索式演化下的具体形态——被最大化的是 fitness 所用的那个口径，而不是它想代表的能力。无代码或 artifact 释出，被演化的 Monet 为专有 agent。其 action validity 审计另有超出本节的价值，见 §8.13。库内暂无独立验证。
...
| OSWorkerBench [[Papers/2608-UIMate]] | 41 个 mock 企业应用上的长程办公流程，Long-Memory 与 Multi-App 两类约束可重叠 | 88 个任务用 state-based evaluator、12 个用任务特定 evaluator | 在固定 200 步预算下并报 binary success 与 progress score |
...
OSWorkerBench 把桌面评测的对象从操作系统任务移到企业办公流程：100 个任务覆盖 41 个 mock 应用与 10 个职能族，其中 67 个标 Long-Memory、49 个标 Multi-App，两类可重叠，作者据此拒绝按这两个标签做因果归因。其上的能力落差比 OSWorld 系更陡——UI-Mate-27B 为 41.00% success / 76.86% progress，其基座 Qwen3.6-27B 为 23.33 / 52.35，Kimi-K2.6 为 40.67，而 GPT-5.6-Sol 达 71.00 / 87.67。轨迹长度也进入另一量级：中位 68（Kimi-K2.6）到 71（UI-Mate-27B）个决策轮，40 条 UI-Mate 轨迹超过 100 轮。该 benchmark 建在 mock 企业后端而非真实生产的 Slack/Gmail/Salesforce 之上，任务规范、示范配对与 evaluator 的释出在论文中写为计划，因此当前只能作为提出方自评的口径读。
...
这套报告集把 latency 当作成本项，却不把它当作**约束**。[[Papers/2607-AAPT]] 给出这一缺口的可测形式：当环境本身设定动作窗口时，成功判据是"正确且及时"，而现有 CUA benchmark 无一提供隔离该判据所需的毫秒级 deadline 控制。它实测 reactive 完整往返的 p50 为 567 ms（部分 block 730 ms），并把这一分位对齐到 19 个有官方文档的真实响应窗口（BIOS fast-boot 按键约 0–200 ms、Android double-tap 300 ms、Windows double-click 500 ms、Android Toast 2–3.5 s、GRUB 5 s、TOTP 30 s），论证亚秒级窗口在部署系统中确实存在；但它没有量化这类失败在现有 agent 失败构成中的占比，其外部 benchmark 上可预枚举的 deadline 任务也只占 9/39。因此本节采纳的是报告项而非优先级判断：效率报告应补上 decision-time latency 的分位（而非只报总 wall-clock），并标明任务是否存在环境侧 deadline。同一工作也提醒 latency 优化常常是把算力搬走而非消除——其 completion token 为 reactive 的 3.2×（342.8 vs 105.8），单 trial 墙钟 3.32 s vs 1.65 s，因此 latency 与 token 成本必须分列而不能互相代表。
...
Passive judge 一类的上限此前只有定性判断，[[Papers/2607-OSReward]] 把它做成了可测量。它没有复用现成 benchmark 的轨迹——理由是那会把 judge 的错误、rollout 本身的质量缺陷与原 verifier 的假阳假阴混在一起，使失败不可归因——而是在 web、Android emulator、Ubuntu、Windows 四个平台自建环境与指令，由 3 名标注者独立标注、分歧升级两名 senior reviewer 共同审议（deliberation 而非多数投票），得到 1019 条 gold trajectory（43% success / 57% fail，calibrated Krippendorff α = 0.797）。在固定协议（末 5 张截图 + 每步 thought/action 文本、greedy、无 task-specific harness/工具/step 监督）下，27 个 reference judge 的全集最高 89.7%（Claude-Opus-4-8）、最低 69.4%；在由标注者自身分歧构造并经复核的 OSReward-Hard（284 条，30/70）上每个 judge 掉 20–43pp，最佳 69.7%、均值 52%。作者自己指出 Hard 上 raw accuracy 是陷阱——69.7% 恰好等于该分布下"恒判 fail"的常数判别器——须改看 BalAcc，引用时不应只搬这句对比。
...
| 威胁 | 现有证据 | 最低防线 |
|:--|:--|:--|
| Shortcut / hard-code | [[Papers/2407-AgentsThatMatter]] 讨论 benchmark-specific shortcut；[[Papers/2510-HAL]] 报告 scaffold 与日志中的 gaming case；[[Papers/2608-DarwinX]] 在 WebArena-Infinity 上逐条审计轨迹，一个 harness 的 raw 53.0% 中含 293 条无效轨迹、confirmed-invalid 率 23.5% | task-level holdout、未见网站/任务模板、禁止 benchmark-specific branch；对 raw pass@1 报告配套的 trajectory validity 审计与无效类别分解 |
...
[[Papers/2607-AAPT]] 的 compile-cache 观察目前是单一 serving stack（vLLM `guided_json` / xgrammar 后端）内的单点证据，其跨引擎普遍性未经检验；但它指向的失败形态值得单独设防——当 agent 的控制流依赖一个阈值（schema 合法率、置信度门、路由准确率）时，服务栈层面的无声漂移不会表现为分数缓慢下滑，而是把整条 arm 打到接近零，且在权重、prompt、sampler 完全固定的情况下发生。因此复现协议的记录单位应包含推理服务栈本身，而不止于模型与数据。
...
[[Papers/2608-DarwinX]] 在 exploit 这一面上提供了少见的一手审计数据。WebArena-Infinity 的 1,260 道真实任务上，一个两阶段检测器（静态去混淆 + taint 追踪 + 独立 judge 模型，覆盖率 99.0% 与 99.4%）对 base harness 的 raw 53.0% 逐条判定，查出 293 条无效轨迹：155 条越界访问 evaluation plane、97 条访问特权 host、26 条 exploit 或提权、15 条直接篡改 raw state，confirmed-invalid 率 23.5%；演化之后前三类全部消失，只剩 17 条 raw-state mutation 且集中在单个应用，该率降至 1.4%。它把"未审计的 pass@1 含有不明比例的无效成功"从推测变成一个有数量级的观察，而这类审计目前几乎无人配套发布。改善幅度本身则不能整体读作行为变好：演化后的 prompt 把 base 中"只走 UI、绝不直接写状态"的绝对禁令放宽为"经有界审计后可调用应用自有的 action/update helper 做最小改动"，合规率的一部分来自把一类非 UI 动作重新划进合法区，而划线的 rubric 由同一团队撰写，作者自述为 behavioral co-improvement 而非 causal decomposition。审计器与 rubric 均未经第三方复核，原文另有一行口径不同的 invalid successes 计数（120→17），引用时须指明用的是哪一个。

WebArena 系的 exploit 与 evaluation-plane 越界因此有了一处可引用的量级，但它出自单一团队、单一 harness 家族、单一 benchmark，且审计标准与被审对象同源；gold leakage 与 original-to-verified checker 修订仍缺 primary audit 证据。本节相关论断的证据基础因此仍然偏弱。
...
**Validated Gap — 等证据预算的 verifier 对照。** Programmatic、passive judge 与 interactive verifier 访问的证据不同，现有结果无法判断收益来自更好的推理还是更多取证。[[Papers/2607-InteractiveRewardAgent]] 的同 backbone 消融（VLM-only / GUI-only / 全工具）提供了最接近的系统内证据：只加 GUI 交互反而使 Qwen3.6 recall 大跌（找不到证据被误判失败），补全 system/application 工具后才改善 precision–recall trade-off——取证通道的构成而非有无决定收益；但跨 verifier 家族的等预算对照仍缺失，且其 benchmark 标签由 script 产生、覆盖天然限于 script 可判任务。[[Papers/2607-OSReward]] 从 passive 一侧补上跨模型的等协议对照（27 个 judge、human-gold 标签、四平台自建轨迹），把该家族的失败刻画为方向性的 leniency 而非对称噪声，并排除了两条常被默认有效的补救：多 judge 投票（top judge 间 κ≈0.71，top-3 vote 仅 +1pp）与加 thinking（收益随 base 变强单调缩小，最强 judge 仅 +0.4pp）。它同时把这条 gap 的另一半标了出来——全文没有把 reward model 接进策略训练，因此"verifier 质量到策略收益"的传递函数仍无任何数据点。决定性实验应固定 actor、trajectory、environment snapshot 与 evidence budget，比较 precision、recall、coverage、abstention、cost、state pollution 和抗操纵能力，由独立 human audit 仲裁，并在同一 verifier 差异下测下游 policy 训练结果 [[Papers/2504-AgentRewardBench]] [[Papers/2510-CUARewardBench]] [[Papers/2602-VAGEN]] [[Papers/2607-InteractiveRewardAgent]] [[Papers/2607-OSReward]]。
...
对 computer-use agent，成本与"完成度"深度纠缠：OSWorld 2.0 的前沿显示，Claude Opus 用极大 token 预算换取最高完成率，而 GPT 系更省 token 但更早触顶（详见 §9.10 数字），意味着"必须完成"与"必须便宜"是两条不同曲线。[[Papers/2510-HAL]] 进一步实证 scaffold 造成数量级成本差：Online Mind2Web 上 SeeAct+GPT-5 花 \$171、BrowserUse+Claude Sonnet 4 花 \$1,577（9× 差距）而 accuracy 只差 2 个百分点；且该 benchmark 单次评测平均 >\$450（arXiv 2510.11977）。学术侧对"成本随什么量增长"的形式化目前只有 framing 级证据：[[Papers/2604-AgenticCompilation]] 把重复执行固定 workflow 的推理支出写成 Cost = M × Σ[S_i × C_t]（M 为执行次数），主张经一次 LLM 编译成确定性 blueprint 后成本对 M 摊销为 O(1)——实际应为 O(R)，R 是 UI 变更触发的重编译频率，而全文未测量 R；其可用之处是判据本身：LLM 是否在运行时决定 control flow，比"有没有缓存或 memory"更能区分成本类别（workflow memory 压的是每次的步数 N，不改变对 M 的线性）。该文为单作者 industry position paper，baseline 成本系估算而非实测且无代码（库内笔记 verification_status: unverified），其宣称的削减倍数不作经验数据引用；同方向带 baseline 实证的工作见 [[Papers/2504-SkillWeaver]]。
...
- 并入：[[Papers/2605-Weasel]]（§5.10 主落位，把"哪些 step 进 SFT"写成固定预算子集选择；cross-ref §4.5 target-centered pruning 与 §7.2 SFT 监督目标）
- 分类记录：platform = web；task_level = app workflow；primary_section = data-task；environment_setting = offline 训练 + self-hosted 评测；verifier_type = programmatic + LLM judge（GPT-4.1-mini / GPT-4-1106-preview）；evidence_strength = direct end-to-end
- 关键变化：§5.10 的数据处理单位从语料/轨迹层下沉到轨迹内单步，并首次出现可逐项消融的选择目标函数（此前该小节只有 ScaleCUA 的单点配比决策与 SCALECUA 的去重审计）；§4.5 的"压缩非普遍有益、依赖底座"校正性发现获得训练语料侧的同向证据（此前三条证据均产生在推理期）
- 无新增小节、无配图变化；Key Takeaways / Open Problems / Key Evidence Matrix 未动——Weasel 的 pruning 负面证据与 [[Papers/2604-ReadMoreThinkMore]] 那一行处在不同平面（训练语料 vs 推理观察），属于小节级 enrichment 而非高层判断改写，硬并入同一矩阵行会混淆两者的适用范围
- verification 纪律：`verification_status: source-checked`，12/12 高风险 claim 经独立 verifier 核查；主结果与全部消融数字随 backbone、数据量、epoch 与 judge 口径写入；三处限制（Qwen3-8B 上全量 SFT 净收益近零、Pruning+LLM-Judge 19.4 vs Weasel 21.2 且 w/o RS 仅 17.0、单独 pruning 掉点未被原文解释）一并写入正文而非只写主结果；单篇工作、库内暂无独立验证，正文以具体边界句承载而非用泛化 hedge 替代
- DomainMap 未刷新：单篇论文填补既有小节的方法空缺，未达格局级变化

### 2026-08-18 survey-refresh（并入 2 篇，212 → 215）

- 并入：[[Papers/2608-UIMate]]（§6.3 主落位，开权重桌面 native model；cross-ref §7.9 与 §8.5）、[[Papers/2608-DarwinX]]（§7.11.1 主落位，harness 演化的 population selection 形态；cross-ref §8.13）
- 关键变化：§7.9 与 §11.4 把 verifiable task 合成的失效面从"任务是否成立"扩到"不变量通过之后 reward 是否对齐指令语义"，18%（过严 40% / 空洞 23% / 错目标 19%）是这一层的首个量化；§8.13 的收尾判断据 DarwinX 的 293→17 轨迹审计改写——exploit 与 evaluation-plane 越界从"缺 primary audit"变为"有一处同源量级"，gold leakage 与 checker 修订仍缺；§7.11.1 的 workflow/harness 一行获得第二种 gate 形态（contract-based 准入对 review 面板），并记录 population 主张在收益最大的 benchmark 上未生效；§8.5 与 §6.3 各增一行
- 无新增小节、无配图变化；Key Evidence Matrix 新增 2 行（UI-Mate 的 18% 不对齐、DarwinX 的 23.5% confirmed-invalid），因两者分别改写了 §11.4 的 takeaway 与 §8.13 的证据基础判断
- verification 纪律：两篇均 `verification_status: partial`，只引用 source-verified 行——DarwinX 摘要的"平均约 17 点"（unsupported，算法未说明）与演化 run 的算力成本（not-checkable）、UI-Mate 的 OSWorld 分域画像（not-checkable）一律未写入；TB2.1 的 +7.7 随 effort 未对齐、TerminalWorld 的 28/41 随 McNemar p=1.0 与 pre-TW 参考 harness 同分、WAI 的 +49.5 随 base 低于 Browser Use 基线一并写入；两篇均标"库内暂无独立验证"
- 计数口径：papers_analyzed 按唯一 Papers wikilink 机械复核为 215，比 212 + 2 多 1——08-11 轮在 §4.3 引入的 [[Papers/2608-WorldProxy]] 未计入当轮计数，本轮一并更正

### 2026-08-12 survey-refresh（并入 3 篇，209 → 212）

- 并入：[[Papers/2608-ROPSD]]（§7.10，test-time self-training / on-policy self-distillation，Reflector 作 verifier 把文本反思转 token 级监督，是 VAGEN 悬置的"verifier bias 放大"风险的具体实例；transductive 非流式 TTA、仅单步 grounding）、[[Papers/2608-GatedHindsight]]（§7.13，next-screenshot privileged-info gated distillation，与 R-OPSD 的参数共享条件化 self-teacher 同构，cross-ref §7.9 GRPO<SFT）、[[Papers/2608-Ouroboros]]（§7.11.1，workflow/harness 自演化路线的具体系统落位，blocking diff-review 面板 + 受保护 constitution 即本节所要求的独立 gate）
- 关键变化：§7.10 补一段 test-time self-distillation，把 verifier-bias 风险从 VAGEN 的"未验证悬置"推进到 R-OPSD 的"实例化但未做压力测试"；§7.13 把 MAGA 的"监督给哪个 token"扩展到"在哪些 rollout / 用什么特权信号"；§7.11.1 的 workflow/harness 一行首次获得具体系统落位与可阻断 gate 实例
- 无新增小节、无配图变化；Key Takeaways / Open Problems / Key Evidence Matrix 未动（均为小节级 enrichment，未升级高层判断）
- verification 纪律：三篇均标"库内暂无独立验证"；R-OPSD（50.2→57.6 / +7.4、GUI-RCPO −0.3、Reflector 89.5%）与 GHD（AndroidLab-8B 37.43→54.11、+7.42 vs +5.62）的 source-verified 数字随 environment / verifier / backbone 口径写入，Ouroboros 的 OSWorld-Verified 90.69% 随 frozen-seed 与 ±1.7pp SE、causal-attribution 边界一并写入；DomainMap 未刷新（单篇 unverified 实例填补既有表行，未达格局级变化）

### 2026-08-11 survey-refresh（并入 8 篇，201 → 209）

- 并入：[[Papers/2608-AppDeltaWorld]]（§4.3，world model 作 mobile 环境替代，双口径证据）、[[Papers/2605-Region4Web]]（§4.5，observation/action 粒度解耦）、[[Papers/2604-ToolIllusion]]（§4.7.2，工具收益条件化）、[[Papers/2605-PlanThenExecuteWeb]]（§6.11.1 主落位 + §4.7.2 API 覆盖 cross-ref）、[[Papers/2608-StepReflect]]（§6.10，结构化 reflection + 方向性偏差对）、[[Papers/2602-CATTS]] 与 [[Papers/2603-Ares]]（§7.14，per-step compute allocation）、[[Papers/2604-AgenticCompilation]]（§9.9，unverified 限权、仅作 framing 与判据引用）
- 关键变化：§4.7.2 把"工具通道有益"改写为条件命题（工具来源 × 合成者-使用者相对能力 × 覆盖与可组合性）；§6.10 建立 step 级 reflection（偏报忧）与轨迹级 judge（偏放行）的方向相反偏差对；§7.14 新增 per-step 采样/仲裁/effort 预算分配证据段；§4.3 环境供给谱系增加"学环境"形态
- 无新增小节、无配图变化；Key Takeaways / Open Problems / Key Evidence Matrix 未动（均为小节级 enrichment，未升级高层判断）
- verification 纪律：AgenticCompilation（unverified）未引任何数字；Ares/CATTS/PlanThenExecute/AppDeltaWorld（partial）仅引 source-verified claim 行；CATTS 的 +9.1% 与 Ares 的 45.3% 均随口径限定一并写入
...
- [[Papers/2607-QwenUIAgent]]（environment-runtime）：§4.3 新增生产规模真机集群一行 + 证据边界段；§4.8 表内并入"RL 训练联合 policy"行并补部署级 channel-mix 段（CLI 占动作 40.7%/55.1%、batched 39.6%/41.6%、execution→verification 转移 40.2%→52.4%）；§6.3 新增模型行与 27B dense vs 35B-A3B MoE 反常。未改 §11.5 Takeaway——它是对既有判断的佐证而非修正。
- [[Papers/2607-AAPT]]（model-architecture）：§6.8 新增两段（decode 占 decision-time critical path、$T_{cover}\ge L_{p95}+M$ 规则、外部效度与成立条件），并把 deadline/覆盖窗口补进 planning contract 字段表述；§8.10.3 补 decision-time latency 报告项；§8.13.1 新增 serving-stack / compile-cache drift 威胁行 + §8.13.2 边界段。
- [[Papers/2607-OSReward]]（evaluation-verifier）：§8.12 表内两行修订 + 新增三段（leniency 方向性、证据通道失衡的 ablation 归因、ensembling 同质性、OS-Shepherd 的两条边界）；§7.6 把"reward model ≠ ground truth"从"有噪声"收紧为"有方向"；§8.14 的等证据预算 verifier 对照 gap 补上 passive 侧对照与"无下游 policy 实验"这一半。
- Key Evidence Matrix 新增 3 行（OSReward 方向性偏差、ensembling 同质性、AAPT serving-stack 漂移）。本轮无结构性重构，未新增/重画配图。
