...
**Weaknesses / 边界**
- STEPO 的均匀分摊仍是**粗粒度 credit assignment**——修复的是 group 平衡偏差，不是 per-step 归因；作者自己承认 terminal reward 稀疏下定位关键动作仍未解决（PRM 单独使用会被 hack 而不可靠，milestone/counterfactual 只是方向）。
