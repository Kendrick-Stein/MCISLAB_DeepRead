...
> Vendored from [EvoScientist/EvoSkills](https://github.com/EvoScientist/EvoSkills) (Apache-2.0), adapted for ReadPaperMachine（工具名/路径/交叉引用/Guard/Verify 本地化）。License: references/licenses/EvoSkills-Apache-2.0.txt
...
给定一个研究主题或种子论文，构建"挑战 → 方案 → 论文演进路径"的 lineage 图，产出带 Mermaid 图的 Markdown 报告，帮助梳理某个方向的历史脉络。输入是自由文本的研究 query（主题、种子论文标题/引用，或二者混合）与输出路径；输出是单个 Markdown 文件（taxonomy 图 + 每个 solution 的演进图 + 论文附录）。

该报告是 `DomainMaps/` 的补充材料，而非替代品：图中呈现的是可追溯到引用关系或明确方法继承的演进路径，若报告中的发现值得并入某个 Domain Map，在报告末尾标注"建议加入 DomainMaps"，交给 `Workbench/queue.json` 走 Review 流程——不直接修改 `DomainMaps/` 下的文件（`DomainMaps/` 的直接写入仅限 `survey-refresh` 的 `## 近期格局变化` 小节）。

**vault 内运行时的数据来源优先级**：本 skill 原生的自动化 runbook（下方 CLI 步骤）依赖 Semantic Scholar API（`S2_API_KEY`，必需）与可选的 DeepXiv SDK，两者都需要额外安装 `httpx` / `python-dotenv`（/ `deepxiv-sdk`，未纳入本 vault `pyproject.toml`，需要用户自行 `pip install` 到当前 Python 环境）并联网调用外部服务——在网络受限的沙箱环境下这条自动化链路可能不可用。因此在 vault 内使用本 skill 时，**优先直接用 `Papers/` 中已读论文的笔记内容（cite_key、引用关系、方法继承关系）手工/由 host agent 推理构图**，只在需要发现库外论文、且用户已自行配置好上述依赖与 API key 时，才走下方完整的 CLI runbook 去调用 S2/DeepXiv 做自动检索。无论走哪条路径，图中每篇论文都应标注是否已在 `Papers/` 库内（已收录 / 库外）。
...
**Python deps (not in this vault's `pyproject.toml` — install separately before running the CLI runbook):** `httpx`, `python-dotenv`. Neither is a vault runtime dependency; install them into whatever Python environment runs `scripts/cli.py` (e.g. `pip install httpx python-dotenv`).
...
**Network:** `scripts/cli.py` makes direct outbound HTTP calls (via `httpx`) to `api.semanticscholar.org` and (if configured) the DeepXiv API. This is only reachable when the running environment allows outbound network access — see the vault-priority note above for the fallback (build from `Papers/` instead).
...
```bash
python3 skills/7-presentation/paper-graph/scripts/cli.py resolve_seed_papers \
...
```bash
python3 skills/7-presentation/paper-graph/scripts/cli.py format_seed_block \
...
Read `skills/7-presentation/paper-graph/references/parse_query.md`. Substitute `{seed_block}` with the contents of `<workdir>/seed_block.txt` and `{query}` with the contents of `<workdir>/query.txt`. Call the LLM (low temperature, ~0.1).
...
```bash
python3 skills/7-presentation/paper-graph/scripts/cli.py fetch_papers \
...
```bash
python3 skills/7-presentation/paper-graph/scripts/cli.py format_papers \
...
```bash
python3 skills/7-presentation/paper-graph/scripts/cli.py merge_classifications \
...
```bash
python3 skills/7-presentation/paper-graph/scripts/cli.py prefetch_sections \
...
```bash
python3 skills/7-presentation/paper-graph/scripts/cli.py build_goal_block \
    --parsed-query <workdir>/parsed_query.json \
    --out <workdir>/goal_block.txt

python3 skills/7-presentation/paper-graph/scripts/cli.py compute_core_filter \
    --papers <workdir>/papers.json \
...
```bash
python3 skills/7-presentation/paper-graph/scripts/cli.py parse_outline \
...
```bash
python3 skills/7-presentation/paper-graph/scripts/cli.py parse_detail \
...
```bash
python3 skills/7-presentation/paper-graph/scripts/cli.py render_outline_mermaid \
    --raw <workdir>/outline_raw.md \
    --papers <workdir>/papers.json \
...
```bash
python3 skills/7-presentation/paper-graph/scripts/cli.py render_detail_mermaid \
    --raw <workdir>/details/<key>_raw.md \
    --context <workdir>/solutions/<key>.json \
    --papers <workdir>/papers.json \
...
```bash
python3 skills/7-presentation/paper-graph/scripts/cli.py assemble_report \
    --parsed-query <workdir>/parsed_query.json \
...
- lineage 图中的每一条论文关系必须有依据（S2/DeepXiv 返回的引用关系，或论文正文中明确陈述的方法继承），不得为了让图"看起来完整"而臆造演进关系或跳过审计（Step 11 `audit_edge`）直接下结论
- vault 内运行时优先用 `Papers/` 中已读论文构图；只有在需要补充库外论文、且用户已自行安装 `httpx`/`python-dotenv`/`deepxiv-sdk` 并配置好 `S2_API_KEY`（及可选 `DEEPXIV_API_TOKEN`）时，才调用下方 CLI runbook 做自动检索联网抓取
- 图中出现的每篇论文都要标注是否已在 `Papers/` 库内收录（已收录 / 库外），不得把库外论文悄悄当作已读论文引用
- 不直接修改 `DomainMaps/` 下的任何文件；报告中值得并入 Domain Map 的发现只能标注"建议加入 DomainMaps"并走 `Workbench/queue.json` 的 Review 流程
- 缺少必需的环境变量（`S2_API_KEY`）或依赖包时，如实报错并说明缺什么，不得静默跳过数据获取步骤产出残缺图谱

## Verify

- [ ] 输出 Markdown 中每个 Mermaid 代码块都以 ` ```mermaid ` 开始、以 ` ``` ` 结束，语法可被 Mermaid 渲染器解析
- [ ] 图中每条边（论文 A → 论文 B）都有对应的说明文字或 audit 结论（`SUPPORTED_BY_ABSTRACT` / `SUPPORTED_BY_SECTION` / `INFERRED`），未标注依据的边不得出现
- [ ] taxonomy 图至少包含 2 个 challenge，每个 challenge 至少 1 个 solution；论文附录存在于文件末尾
- [ ] 输出文件路径与用户指定的路径一致
- [ ] 图中每篇论文已标注是否在 `Papers/` 库内；若有"建议加入 DomainMaps"的发现，已写入报告末尾而非直接改动 `DomainMaps/`
