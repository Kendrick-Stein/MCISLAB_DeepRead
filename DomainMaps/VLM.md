...
- **2026-09-07｜readout gap 从两段延长成三段**：既有认知止步于"表征可解码但行为读不出"，三篇互不相关的工作把链条接长一节——证据被正确取到、甚至被正确判定之后，动作与答案仍然不更新。[[Papers/2608-MNISTPro]] 把同一条探索轨迹已采集的 glimpse 事后拼成一张画布重新呈现，Claude-5-Opus 从 41.0/13.0 回到 76.0/67.0；[[Papers/2608-MemoryLies]] 中所有踩中 stale 格而死的案例都发生在检测 F1 > 0.9 的 run 里，且把学得的 stale 标签换成 oracle 标签在任何 regime 都测不出进一步增益。诊断流程因此多一道分支：判定"读出失败"之后还须区分"读出对了但决策没跟上"，判据是把已有证据换一种形式重新呈现看结果是否恢复。两个环境都完全可观测、证据可按坐标无损对齐，能否迁到证据须靠推断补全的场景尚无数据（[[Topics/VLM-Survey]] §2.8 / Takeaway 13）
- **2026-09-07｜读出的对象可能一直被设错**：[[Papers/2608-GroundingIsntKnowing]] 用 token 消融 + attention knockout + causal mediation 给出方向相反的限定——把物体内部的 visual token 换成常量 embedding 后 localization 掉 5.37~64.93 点而 relation 最多掉 1.32，反倒是向外扩一格吃进周边像素才伤到 relation。若成立，则关系推理依赖粗粒度的 object-centered layout 而非精确边界，"提高定位精度以改善关系理解"这条默认路径是在优化一个不必要的中间量。证据宽度须先看清：三个模型里两个分别贴着四选一的随机水平与天花板，对照只匹配 token 数量、未匹配形状，而被替换区域的轮廓与位置恰好被保留（[[Topics/VLM-Survey]] §2.8 / Takeaway 8）
- **2026-09-07｜免标注 reward 的偏差来源从"自身"移到"cohort"**：[[Papers/2608-CoRLCohort]] 把 [[Papers/2606-VisPlay]] 那条自我多数票路线换成独立预训练 peer 的多数票，并把自我共识的退化写成定理（对奇数 K，self-rewarding 的期望更新方向恒为 sign(p−1/2)）。范式门槛因此改写：从"模型自己的多数票够不够准"变成"两个模型会不会错成同一个答案"，后者随答案空间收缩而恶化，多模态的多选题与小离散动作空间正落在失效侧。等预算对照下增益极不对称（弱 agent +3.19/+4.86，强 agent +0.53/+1.43），主效应更像单向蒸馏而非共识生效（[[Topics/VLM-Survey]] §2.6 / Open Problem 5）
- **2026-09-18｜免标注 reward 的来源阶梯接到第四级，而第四级是被证伪的那一级**：09-07 记下的移动是自身共识 → peer cohort。第四级是专门训练的判别式 verifier，[[Papers/2609-FailBench]] 在 14 个来源、2,197 条轨迹上以相同 harness 与 prompt 对比，5 个专训失败检测器相对各自 base 模型全部为负（−0.002 ~ −0.088），而最好的通用模型 Gemini 3 Flash 也只有 0.77 macro balanced accuracy。退化形态可读：RoboFAC-7B 抓到 30/30 失败却只认出 2/30 成功（BA 0.533），按类别先验重加权后 0.811，与 always-failure 基线 0.797 几乎无差——它学到的是先验而不是判别力。边界：该基准无方差、置信区间与显著性检验，且去掉 in-domain 子集后排序会变（Guardian 0.596 < InternVL3-8B 0.639），结论只对"跨来源、无域内数据"这一设定成立（[[Topics/VLM-Survey]] §2.6）
- **2026-09-18｜离散动作接口的 grounding 来自写明的映射而非动作名的语义，08-05 记下的"接口是一等性质"因此多出一条可操作规则**：[[Papers/2609-ShowHarness]] 的 2×2 消融把命名可读性与约定有无正交开——语义命名 + 书面约定 20/20、仅语义命名 18/20、任意符号 + 书面约定 19/20、仅任意符号 1/20，而模型自行推断的映射只有 23.3% 正确。决定成败的是约定写没写，名字是否可读几乎不影响。这与 [[Papers/2608-MissClick]] 的 digit 位权误差面、[[Papers/2606-TowardsGUIAgents]] 的 anchor→extent 同向：接口的承载结构比它的语义外观更决定行为。边界：仅 20 次试验、无重复；该文主表因 VLA baseline 的动作格式混淆不可用，本条只依据这组消融（[[Topics/VLM-Survey]] §2.7）
- **2026-09-18｜统一或扩展带来的通用多模态回退不能归因于视觉编码器退化，冻结视觉塔不构成保护**：[[Papers/2609-PhysBrain15]] 在 12 项通用多模态上 7 项下降（MVBench −2.46、MME −62.6）；[[Papers/2609-Gander]] 相对 base MiniCPM-o 4.5 在 WorldSense −6.08、Daily-Omni −1.67，而它的视觉塔全程冻结且逐比特未变——后者直接排除了"回退源自视觉侧被训坏"这个默认解释。边界：两篇都没做"同算力、仅通用数据"的对照，因此无法区分这是能力冲突还是单纯的训练分布偏移，而这恰是该判断能否升格的唯一实验（[[Topics/VLM-Survey]] §2.3）
