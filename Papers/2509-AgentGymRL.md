...
- **对 AFE / 环境引擎的证据价值**：与 [[Papers/2511-DreamGym]] 的"4 并发证词"同源但走了相反路线——DreamGym 放弃真实环境，AgentGym-RL 花工程把 WebArena 改造成勉强 RL-ready。改造清单（并行化、full-reset、内存治理）正是 [[Topics/WebEnvironment-Engine-Survey]] 轴 1/轴 3 的需求实例；其"状态不一致累积毁训练"的观察给 reset 需求补了训练侧动机（评测侧动机来自任务间污染）。
